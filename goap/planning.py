        ...
        while frontier.count > 0:
            ...
            for next_node in graph.neighbors(current):
                ...
                if (
                    ...
                    goap_python.agent.Agent.OnEvaluatedActionNode(
                        next_node, self.CameFrom
                    )
...
class Planner:
    ...
    def plan(agent: "Agent", cost_maximum: float, step_maximum: int) -> None:
        ...
        goap_python.agent.Agent.OnPlanningStarted(agent)
        best_plan_utility = 0.0
        best_astar: Optional[ActionAStar] = None
        best_goal: Optional["BaseGoal"] = None
        for goal in agent.Goals:
            goap_python.agent.Agent.OnPlanningStartedForSingleGoal(agent, goal)
            ...
            if cursor is not None:
                ...
                goap_python.agent.Agent.OnPlanningFinishedForSingleGoal(
        ...
        if (
            ...
            agent.IsBusy = True
            goap_python.agent.Agent.OnPlanningFinished(
                agent, best_goal, best_plan_utility
            )
        else:
            goap_python.agent.Agent.OnPlanningFinished(agent, None, 0.0)
    ...
    def _update_agent_action_list(
        ...
        goap_python.agent.Agent.OnPlanUpdated(agent, action_list)
