...
// Browser-compatible UUID generator (simple version)
const uuidv4 = () => {
...
// Simplified pygame mock - just the essentials for the game loop
const pygame = {
    init: () => {},
    display: {
        set_mode: (size) => ({ fill: () => {}, get_rect: () => ({ center: [0, 0] }) }),
        set_caption: (caption) => {},
        flip: () => {},
    },
    time: {
        ...
        if (this.CurrentActionSequences.length > 0) {
            ...
            for (const sequence of this.CurrentActionSequences) {
                if (sequence.length > 0) {
                    ...
                    RpgExampleComparativePygame.addToLog(`${this.Name} executing action: ${action_to_execute.Name}`);
...
class RpgExampleComparativePygame {
    ...
    static run(use_extreme) {
        ...
        if (!this.canvas || !this.ctx) {
            console.error("Canvas not found! Make sure you're running this in a browser with the HTML file.");
            return;
        }
        
        console.log("\n=== RPG GOAP Example (Canvas Version) ===");
        ...
        const gameLoop = () => {
            ...
            if (current_time - last_update >= 500) {
                ...
                if (!agents.includes(player)) {
                    this.addToLog("Player defeated! Game Over.");
                    running = false;
                    return;
                }
            }

            RpgExampleComparativePygame._render_grid(this.canvas, agents, food_positions, turn);
    ...
    static addToLog(message) {
        ...
        if (logElement) {
            logElement.innerHTML = this.gameLog.join('<br>');
            logElement.scrollTop = logElement.scrollHeight;
        }
    }

    static _render_grid(canvas, agents, food_positions, turn) {
        const ctx = this.ctx;
        if (!ctx) return;
        ...
        for (const food_pos of food_positions) {
            if (food_pos instanceof Vector2 && 
                food_pos.X >= 0 && food_pos.X < MaxX && 
                food_pos.Y >= 0 && food_pos.Y < MaxY) {
                const x = food_pos.X * cellWidth + 2;
                const y = food_pos.Y * cellHeight + 2;
                ctx.fillRect(x, y, cellWidth - 4, cellHeight - 4);
            }
        }
        
        // Draw agents
        ctx.font = `${Math.min(cellWidth, cellHeight) * 0.4}px Arial`;
        ...
        for (const agent of agents) {
            ...
            if (pos instanceof Vector2 && 
                ...
                ctx.fillText(hp.toString(), centerX, centerY);
            }
        }
        
        // Update UI stats
        this._update_ui_stats(agents, food_positions, turn);
    }
    
    static _update_ui_stats(agents, food_positions, turn) {
        ...
        const players = agents.filter(a => a.State["faction"] === "player");
        const monsters = agents.filter(a => a.State["faction"] === "enemy");
        
        if (playerStats && players.length > 0) {
            const player = players[0];
            const food_eaten = player.State["food_eaten"] || 0;
            const hp = player.State["hp"] || 0;
            const pos = player.State["position"];
            playerStats.textContent = `Player: HP=${hp}, Food=${food_eaten}, Pos=(${pos.X},${pos.Y})`;
        }
        
        if (gameInfo) {
            gameInfo.textContent = `Monsters: ${monsters.length}, Food: ${food_positions.length}`;
        }
    }
...
// === MAIN EXECUTION ===
// Canvas version - executed when HTML page loads
// Comment out the line below and call from HTML instead
