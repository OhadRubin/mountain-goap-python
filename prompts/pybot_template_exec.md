...
<state_machines>
# Main Activity State Machine Conversion Plan

## Overview
The Main Activity State Machine is the highest-level state machine in the Minecraft bot codebase that governs the bot's overall behavior and current primary task. This document provides a detailed natural language plan for converting the current implicit state management to an explicit Automat state machine.

## 1. Current State Analysis

### Implicit State Management
The current implementation uses a combination of boolean flags, string identifiers, and timing variables to manage state implicitly:

- **`activity_major`**: A boolean flag indicating whether a major activity is currently running
- **`stopActivity`**: A boolean flag used to signal termination of the current activity
- **`activity_name`**: A string identifier for the current activity ("None" when idle)
- **`activity_start`**: A timestamp marking when the current activity began
- **`activity_last_duration`**: Duration of the last completed activity

### Control Flow Pattern
The state transitions are managed through two primary methods:
- **`startActivity(name)`**: Transitions from idle to an active state
- **`endActivity()`**: Transitions from any active state back to idle

### Command Processing
The `commandList` dictionary maps string commands to function calls, with each command having metadata about whether it represents a major activity. Major activities cannot be started while another major activity is running, enforcing mutual exclusion.

### Concurrency Model
Major activities are executed asynchronously using the `@AsyncTask` decorator, allowing the bot to remain responsive to chat commands while performing long-running tasks.

## 2. State Identification

### Primary States
Based on the command list and activity management, the following distinct states can be identified:

1. **IDLE**: No major activity is running
   - `activity_major` is False
   - `stopActivity` is True
   - Bot is ready to accept new major activity commands

2. **BUILDING**: Executing a blueprint construction task
   - Triggered by "build" command
   - Involves multi-phase construction process
   - Can be interrupted by stop command

3. **CHOPPING_WOOD**: Harvesting wood resources
   - Triggered by "chop" command
   - Involves finding and cutting down trees
   - Can be interrupted by stop command

4. **FARMING**: Managing crop cultivation
   - Triggered by "farm" command
   - Involves planting, harvesting, and resource management
   - Can be interrupted by stop command

5. **MINING**: Extracting underground resources
   - Triggered by "mine" command
   - Involves tunnel creation and ore extraction
   - Can be interrupted by stop command

6. **FOLLOWING**: Tracking and following a player
   - Triggered by "follow" command
   - Involves continuous pathfinding to player location
   - Can be interrupted by stop command

### Transient States
Additional states that represent temporary conditions:

7. **STOPPING**: Transitioning from active state to idle
   - Occurs when `stopActivity` is set to True
   - Involves cleanup operations and resource management

8. **STARTING**: Transitioning from idle to active state
   - Brief state during activity initialization
   - Sets up timing, flags, and initial conditions

## 3. Event Discovery

### External Events (User Commands)
Events triggered by chat commands processed through `handleCommand`:

1. **build_requested**: User issues "build" command with blueprint argument
2. **chop_requested**: User issues "chop" command
3. **farm_requested**: User issues "farm" command
4. **mine_requested**: User issues "mine" command with location argument
5. **follow_requested**: User issues "follow" command with optional target
6. **stop_requested**: User issues "stop" command
7. **come_requested**: User issues "come" or "go to" command
8. **move_requested**: User issues "moveto" command with location

### Internal Events
Events generated by the system itself:

1. **activity_completed**: Major activity finishes successfully
2. **activity_failed**: Major activity encounters unrecoverable error
3. **activity_interrupted**: External interruption (e.g., health emergency)
4. **initialization_complete**: Bot startup sequence finished

### Minor Command Events
Events for non-major activities that don't change the primary state:

1. **status_requested**: User requests status information
2. **inventory_requested**: User requests inventory listing
3. **deposit_requested**: User requests chest deposit
4. **eat_requested**: User requests food consumption
5. **sleep_requested**: User requests sleep action
6. **wake_requested**: User requests wake action
...
#### From IDLE State:
- **build_requested** → STARTING → BUILDING
- **chop_requested** → STARTING → CHOPPING_WOOD
- **farm_requested** → STARTING → FARMING
- **mine_requested** → STARTING → MINING
- **follow_requested** → STARTING → FOLLOWING
- **Minor commands** → IDLE (self-transition with side effects)

#### From Active States (BUILDING, CHOPPING_WOOD, FARMING, MINING, FOLLOWING):
- **stop_requested** → STOPPING → IDLE
- **activity_completed** → STOPPING → IDLE
- **activity_failed** → STOPPING → IDLE
- **activity_interrupted** → STOPPING → IDLE
- **Major activity requests** → REJECTED (guarded transition)

#### From STARTING State:
- **initialization_complete** → Target active state
- **stop_requested** → STOPPING → IDLE (rare edge case)

#### From STOPPING State:
- **cleanup_complete** → IDLE

### Guard Conditions
- Major activity commands are rejected when `activity_major` is True
- Some commands require minimum argument counts (stored in commandList metadata)
- Certain commands may require specific game states (e.g., bed availability for sleep)

## 5. Output Identification

### Transition Outputs (Actions)
Actions that occur during state transitions:

1. **Activity Initialization**:
   - Set timing variables (`activity_start`)
   - Update activity name
   - Set control flags (`activity_major`, `stopActivity`)
   - Clear danger states
...
3. **Command Execution**:
   - Execute the specific function associated with each command
   - Handle argument parsing and validation
   - Manage asynchronous task creation

4. **Error Handling**:
   - Reject invalid commands with appropriate messages
   - Handle concurrent activity attempts
   - Manage missing arguments or invalid parameters

### State-Based Outputs
Actions that occur while in specific states:

1. **IDLE State**:
   - Respond to status queries
   - Accept new activity commands
   - Perform minor maintenance tasks

2. **Active States**:
   - Execute primary activity logic
...
1. **Create Automat Machine Class**: Define a new class inheriting from Automat's machine base
2. **Define State Set**: Enumerate all identified states as machine states
3. **Define Input Set**: Create input methods for each identified event
4. **Create Initial State**: Set IDLE as the initial state

### Phase 2: Transition Logic Implementation
1. **Map Simple Transitions**: Implement straightforward state changes (e.g., IDLE → BUILDING)
2. **Add Guard Conditions**: Implement validation logic for major activity mutual exclusion
3. **Handle Complex Transitions**: Implement multi-step transitions (STARTING → target state)
4. **Add Error Transitions**: Implement failure paths and error handling

### Phase 3: Output Integration
1. **Extract Output Methods**: Separate side effects from transition logic
2. **Implement Transition Outputs**: Create methods for initialization, cleanup, and execution
3. **Preserve Existing Interfaces**: Ensure command methods maintain their current signatures
4. **Add State Introspection**: Implement methods to query current state and activity status

### Phase 4: Command Integration
1. **Refactor Command Handling**: Modify `handleCommand` to use state machine inputs
2. **Preserve Command Metadata**: Maintain existing command list structure for compatibility
3. **Update Async Task Management**: Ensure asynchronous execution works with state machine
4. **Implement State Validation**: Add checks to ensure commands are valid in current state

### Phase 5: Testing and Validation
1. **Create State Transition Tests**: Verify all valid transitions work correctly
2. **Test Invalid Transitions**: Ensure rejected transitions behave properly
3. **Validate Concurrent Operations**: Test interaction with other state machines
4. **Performance Testing**: Ensure state machine overhead is acceptable

### Phase 6: Migration Strategy
1. **Parallel Implementation**: Run both old and new systems simultaneously
2. **Gradual Cutover**: Migrate commands one at a time
3. **Rollback Plan**: Maintain ability to revert to original implementation
4. **Monitoring and Logging**: Add detailed logging for state transitions

## 7. Edge Cases and Special Considerations

### Concurrency Issues
- **Multiple Command Processing**: Ensure state machine is thread-safe for concurrent chat commands
- **Async Task Interaction**: Handle cases where async tasks complete after state changes
- **Resource Contention**: Manage shared resources between state machine and other components

### Error Recovery
- **Partial State Transitions**: Handle cases where state changes are incomplete
- **External Interruptions**: Manage health emergencies and other urgent conditions
- **Network Disconnections**: Preserve state across connection losses

### Legacy Command Support
- **Non-Standard Commands**: Handle "come", "follow", "moveto" commands that don't fit the standard pattern
- **Argument Parsing**: Maintain existing argument handling for commands with parameters
- **Response Consistency**: Ensure error messages and responses remain consistent

### State Persistence
- **Session Recovery**: Consider whether state should persist across bot restarts
- **Activity History**: Maintain activity duration tracking and reporting
- **Debug Information**: Preserve existing debug output and logging

### Integration Points
- **Survival State Machine**: Ensure proper interaction with health/combat systems
- **Task Sub-machines**: Maintain compatibility with farming, mining, and building state machines
- **Inventory Management**: Preserve existing chest and inventory interaction patterns

### Performance Considerations
- **State Transition Overhead**: Minimize performance impact of explicit state machine
- **Memory Usage**: Ensure state machine doesn't significantly increase memory footprint
- **Response Latency**: Maintain responsiveness to chat commands

### Documentation and Maintenance
- **State Diagram Generation**: Create visual representations of the state machine
- **Code Documentation**: Document all states, events, and transitions thoroughly
- **Future Extensibility**: Design the state machine to easily accommodate new commands and states

## Implementation Notes

### Automat-Specific Considerations
- Use Automat's `@input` decorator for event methods
- Implement proper error handling for invalid state transitions
- Utilize Automat's visualization capabilities for debugging
- Consider using Automat's serialization features for state persistence

### Backward Compatibility
- Maintain existing method signatures where possible
- Preserve command response formats and messages
- Ensure existing scripts and integrations continue to work
- Document any breaking changes clearly

### Testing Strategy
- Create comprehensive test suites for all state transitions
- Test edge cases and error conditions thoroughly
- Validate performance under normal and stress conditions
- Ensure compatibility with existing bot infrastructure

This conversion plan provides a roadmap for transforming the implicit state management in the Main Activity State Machine into an explicit, well-defined Automat state machine while preserving existing functionality and ensuring robust operation.-e \n
# Wood Chopping State Machine Conversion Plan

## Overview
This document provides a detailed natural language plan for converting the Wood Chopping State Machine from its current implicit implementation in `gather.py` to an explicit state machine using the Automat library. The current implementation is primarily contained within the `chopBigTree()` method of the `GatherBot` class.

## 1. Current State Analysis

### Implicit State Management
The Wood Chopping State Machine currently manages its states through a combination of:
- **Program execution flow**: The main state transitions are controlled by the sequential execution within the `chopBigTree()` method
- **Coordinate-based state tracking**: The `y` coordinate variable serves as the primary state indicator, tracking vertical progress through the tree
- **Conditional branches**: State transitions are determined by conditional checks (e.g., `y+8 >= box.y_max` to transition from climbing to descending)
- **Loop structures**: Two main loops control the climbing phase (while True loop) and descending phase (while y >= box.y_min loop)

### Current State Variables
- `y`: Primary state variable tracking current vertical position in the tree
- `box`: BoundingBox object containing tree dimensions and coordinates
- `x0`, `z0`: Base coordinates of the tree trunk
- `self.stopActivity`: External interrupt flag that can terminate the entire process
- Implicit program counter position within the method execution

### State Coupling Issues
The current implementation tightly couples state management with business logic, making it difficult to:
- Resume operations after interruption
- Test individual state behaviors in isolation
- Modify the chopping algorithm without affecting state transitions
- Handle error conditions gracefully
- Provide clear visibility into current operational status

## 2. State Identification

### Primary States
Based on analysis of the `chopBigTree()` method execution flow, the following distinct states have been identified:

#### IDLE
- **Description**: Initial state before any tree chopping operation begins
- **Behavior**: No active tree chopping operations
- **Entry conditions**: Default state or after completing/aborting a chopping operation
- **Valid operations**: Starting a new chopping operation

#### SEARCHING_FOR_TREE
- **Description**: Actively searching for a suitable spruce tree to chop
- **Behavior**: Scans nearby area for spruce logs using `findClosestBlock()` method
- **Duration**: Until suitable tree found or search fails
- **Success criteria**: Finding a spruce log block within specified radius (25 blocks XZ, 1 block Y)
- **Failure conditions**: No suitable trees found in search area

#### VALIDATING_TREE
- **Description**: Analyzing found tree to ensure it meets size requirements
- **Behavior**: Creates BoundingBox to determine tree dimensions and validates against criteria
- **Validation criteria**: Must be exactly 2x2 trunk with minimum height of 5 blocks
- **Success path**: Tree meets dimensional requirements
- **Failure path**: Tree dimensions are invalid

#### PREPARING_TO_CLIMB
- **Description**: Positioning bot and preparing for the climbing phase
- **Behavior**: Moves to optimal starting position, creates access steps, initializes coordinate variables
- **Setup actions**: Walk to tree base, chop access blocks, position for climbing
- **Completion criteria**: Bot is positioned at tree base ready to begin spiral climb

#### CLIMBING_AND_CUTTING
- **Description**: Spiraling up the tree trunk while chopping blocks
- **Behavior**: Follows a specific spiral pattern: chop ahead, move forward, repeat in 4-block cycles
- **Pattern sequence**: 
  1. Chop blocks at (x0, y+1, z0) - south face
  2. Move to and chop at (x0+1, y+2, z0) - east face  
  3. Move to and chop at (x0+1, y+3, z0+1) - north face
  4. Move to and chop at (x0, y+4, z0+1) - west face
  5. Increment y by 4 and repeat
- **Termination condition**: When `y+8 >= box.y_max` (approaching tree top)

#### DESCENDING_AND_CUTTING
- **Description**: Chopping remaining tree blocks from top to bottom
- **Behavior**: Systematically removes all remaining tree blocks level by level
- **Pattern**: Chop all four trunk blocks at current Y level, then decrement Y
- **Block sequence per level**: (x0,y,z0), (x0+1,y,z0), (x0+1,y,z0+1), (x0,y,z0+1)
- **Completion criteria**: All blocks removed down to original tree base level

#### COMPLETED
- **Description**: Tree chopping operation successfully finished
- **Behavior**: Operation completed successfully, ready to return to idle state
- **Final actions**: Activity refresh, return success status

#### ERROR
- **Description**: Operation failed due to error condition
- **Behavior**: Handle various error scenarios gracefully
- **Error types**: Tree not found, invalid dimensions, interruption by stopActivity flag
- **Recovery actions**: Cleanup, status reporting, return failure status

## 3. Event Discovery

### External Events (Method Calls)
These events originate from outside the state machine:

#### START_CHOPPING
- **Trigger**: Call to `chopBigTree()` method
- **Parameters**: None (uses instance variables)
- **Effect**: Initiates the tree chopping sequence
- **Valid from states**: IDLE

#### STOP_ACTIVITY
- **Trigger**: `self.stopActivity` flag set to True
- **Parameters**: None (global flag)
- **Effect**: Immediately terminates current operation
- **Valid from states**: Any active state
- **Behavior**: Graceful shutdown of current operation

### Internal Events (Condition-Based)
These events are generated internally based on environmental conditions or operation results:

#### TREE_FOUND
- **Trigger**: `findClosestBlock()` successfully locates a spruce log
- **Parameters**: Block object with position information
- **Condition**: Spruce log found within search radius
- **Effect**: Advances to tree validation phase

#### TREE_NOT_FOUND
- **Trigger**: `findClosestBlock()` returns None or empty result
- **Parameters**: None
- **Condition**: No suitable trees in search area
- **Effect**: Terminates operation with failure

#### TREE_VALID
- **Trigger**: BoundingBox analysis confirms proper tree dimensions
- **Parameters**: BoundingBox object with validated dimensions
- **Condition**: Tree dimensions are 2x2 trunk with height >= 5
- **Effect**: Proceeds to climbing preparation

#### TREE_INVALID
- **Trigger**: BoundingBox analysis reveals incorrect tree dimensions
- **Parameters**: Actual dimensions that failed validation
- **Condition**: Tree dimensions don't match 2x2 trunk or insufficient height
- **Effect**: Terminates operation with error

#### PREPARATION_COMPLETE
- **Trigger**: Bot successfully positioned at tree base with access cleared
- **Parameters**: Starting coordinates (x0, y, z0)
- **Condition**: Bot positioned correctly and initial blocks chopped
- **Effect**: Begins climbing phase

#### CLIMBING_CYCLE_COMPLETE
- **Trigger**: Completion of one full 4-block spiral cycle
- **Parameters**: Current Y position after cycle
- **Condition**: Successfully chopped and moved through all 4 positions in cycle
- **Effect**: Continues climbing or transitions to descending based on height

#### TOP_REACHED
- **Trigger**: Climbing termination condition met (`y+8 >= box.y_max`)
- **Parameters**: Final climbing Y position
- **Condition**: Bot has reached sufficient height to begin descending phase
- **Effect**: Transitions to descending and cutting phase

#### LEVEL_COMPLETE
- **Trigger**: All four trunk blocks at current Y level have been chopped
- **Parameters**: Current Y level processed
- **Condition**: All blocks at (x0,y,z0), (x0+1,y,z0), (x0+1,y,z0+1), (x0,y,z0+1) removed
- **Effect**: Continues to next level down or completes if at base

#### DESCENT_COMPLETE
- **Trigger**: All tree blocks removed down to base level
- **Parameters**: Final Y position (should equal box.y_min)
- **Condition**: Y coordinate has reached original tree base level
- **Effect**: Successfully completes tree chopping operation

### Error Events

#### OPERATION_ERROR
- **Trigger**: Unexpected error during any phase (block dig failure, movement failure, etc.)
- **Parameters**: Error details and context
- **Condition**: Any operation fails unexpectedly
- **Effect**: Transitions to error handling state
...
#### From IDLE
- **START_CHOPPING** → SEARCHING_FOR_TREE
  - Condition: Always valid
  - Action: Initialize search parameters, begin tree scanning

#### From SEARCHING_FOR_TREE  
- **TREE_FOUND** → VALIDATING_TREE
  - Condition: `findClosestBlock()` returns valid block
  - Action: Create BoundingBox, begin dimension analysis
- **TREE_NOT_FOUND** → ERROR
  - Condition: No suitable trees found in search area
  - Action: Log error, set failure status
- **STOP_ACTIVITY** → IDLE
  - Condition: `self.stopActivity` flag set
  - Action: Abort search, cleanup state

#### From VALIDATING_TREE
- **TREE_VALID** → PREPARING_TO_CLIMB
  - Condition: Tree dimensions meet requirements (2x2, height >= 5)
  - Action: Initialize climbing parameters, move to tree base
- **TREE_INVALID** → ERROR
  - Condition: Tree dimensions incorrect
  - Action: Log validation failure details
- **STOP_ACTIVITY** → IDLE
  - Condition: External termination requested
  - Action: Abort validation, cleanup

#### From PREPARING_TO_CLIMB
- **PREPARATION_COMPLETE** → CLIMBING_AND_CUTTING
  - Condition: Bot positioned correctly, access blocks cleared
  - Action: Begin spiral climbing algorithm
- **OPERATION_ERROR** → ERROR
  - Condition: Unable to position or clear access
  - Action: Handle preparation failure
- **STOP_ACTIVITY** → IDLE
  - Condition: External termination requested
  - Action: Abort preparation, cleanup positioning

#### From CLIMBING_AND_CUTTING
- **CLIMBING_CYCLE_COMPLETE** → CLIMBING_AND_CUTTING (self-transition)
  - Condition: Cycle complete and not yet at top (`y+8 < box.y_max`)
  - Action: Continue with next climbing cycle, increment Y position
- **TOP_REACHED** → DESCENDING_AND_CUTTING
  - Condition: Climbing termination condition met (`y+8 >= box.y_max`)
  - Action: Position for descent, initialize descent parameters
- **OPERATION_ERROR** → ERROR
  - Condition: Movement or chopping failure during climb
  - Action: Handle climbing error
- **STOP_ACTIVITY** → IDLE
  - Condition: External termination requested
  - Action: Abort climbing, cleanup position

#### From DESCENDING_AND_CUTTING
- **LEVEL_COMPLETE** → DESCENDING_AND_CUTTING (self-transition)
  - Condition: Current level cleared and more levels remain (`y > box.y_min`)
  - Action: Move to next level down, continue descent
- **DESCENT_COMPLETE** → COMPLETED
  - Condition: All levels processed down to base (`y <= box.y_min`)
  - Action: Finalize operation, prepare completion status
- **OPERATION_ERROR** → ERROR
  - Condition: Block chopping failure during descent
  - Action: Handle descent error
- **STOP_ACTIVITY** → IDLE
  - Condition: External termination requested
  - Action: Abort descent, cleanup

#### From COMPLETED
- **Automatic** → IDLE
  - Condition: Always triggers immediately
  - Action: Reset state machine for next operation

#### From ERROR
- **Automatic** → IDLE
  - Condition: Always triggers immediately  
  - Action: Cleanup error state, prepare for next operation

### Invalid Transitions
- Direct transitions between CLIMBING_AND_CUTTING and VALIDATING_TREE (must go through proper sequence)
- Any transition that skips mandatory validation phases
- Transitions from COMPLETED or ERROR to active states (must return to IDLE first)

## 5. Output Identification

### State Entry Actions
Actions that occur when entering each state:

#### SEARCHING_FOR_TREE Entry
- Display activity status: "❓ searching: giant spruce"
- Log debug message about tree search
- Initialize search parameters

#### VALIDATING_TREE Entry  
- Create BoundingBox object for dimensional analysis
- Log tree position coordinates
- Validate tree dimensions against requirements

#### PREPARING_TO_CLIMB Entry
- Display activity status with tree location and height
- Walk to optimal starting position relative to tree base
- Clear access blocks by chopping initial positions
- Initialize coordinate variables (x0, y, z0)

#### CLIMBING_AND_CUTTING Entry
- Wield stone axe tool
- Display climbing status with progress indicators
- Begin spiral climbing pattern

#### DESCENDING_AND_CUTTING Entry
- Log "At the top, chopping down" message
- Consume food for health regeneration
- Initialize descent from tree top
- Display descending status
...
#### ERROR Entry
- Log specific error message based on failure type
...
#### TREE_FOUND Actions
- Log found tree coordinates
- Create BoundingBox object
- Store tree reference for subsequent operations

#### TREE_VALID Actions
- Log tree dimensions confirmation
- Display detailed tree information (location, height)
- Calculate and store base coordinates

#### PREPARATION_COMPLETE Actions
- Execute initial chopping sequence for access
- Position bot at climbing start location
- Initialize vertical progress tracking

#### CLIMBING_CYCLE_COMPLETE Actions
- Update activity display with current progress
- Add brief delay (0.5 seconds) between cycles
- Increment Y coordinate for next cycle

#### TOP_REACHED Actions
- Transition vertical position to tree top
- Prepare for systematic descent pattern

#### LEVEL_COMPLETE Actions
- Update activity display with descent progress
- Trigger health check and healing if needed
- Decrement Y coordinate for next level

#### DESCENT_COMPLETE Actions
- Final activity status update
- Prepare success return value

### Continuous Actions
Actions that occur during state execution:

#### During CLIMBING_AND_CUTTING
- Chop blocks in spiral pattern: (x0,y+1,z0) → (x0+1,y+2,z0) → (x0+1,y+3,z0+1) → (x0,y+4,z0+1)
- Move to appropriate position after each chop
- Update activity display with current tree level
- Monitor for termination conditions

#### During DESCENDING_AND_CUTTING
- Chop all four blocks at current level: (x0,y,z0), (x0+1,y,z0), (x0+1,y,z0+1), (x0,y,z0+1)
- Update activity display with descent progress
- Execute healing routine between levels
- Monitor for completion condition

### Error Handling Actions
- Log specific error details for debugging
- Clean up partial state (tool wielding, position)
- Return appropriate failure indicators
- Update activity display with error status

## 6. Conversion Plan

### Phase 1: State Machine Definition
1. **Define Automat State Machine Class**
   - Create `WoodChoppingStateMachine` class inheriting from appropriate Automat base
   - Define all identified states as class-level state objects
   - Establish initial state as IDLE

2. **Define Input Events**
   - Create input methods for each identified event
   - Map external method calls to appropriate input events
   - Ensure event signatures accommodate necessary parameters

3. **Define Output Methods** 
   - Create separate output methods for each identified action
   - Separate state management logic from business logic
   - Ensure outputs are idempotent where possible

### Phase 2: Transition Definition
1. **Map Simple Transitions**
   - Start with straightforward sequential transitions
   - Define transitions without complex guard conditions first
   - Test basic state flow before adding complexity

2. **Add Conditional Transitions**
   - Implement guard conditions for complex transitions
   - Handle branching logic based on environmental conditions
   - Ensure all edge cases are covered

3. **Implement Self-Transitions**
   - Add self-transitions for continuing operations (climbing cycles, descent levels)
   - Ensure proper action execution during self-transitions

### Phase 3: Integration Preparation
1. **Create State Machine Instance Management**
   - Determine where state machine instance will be stored (likely in GatherBot)
   - Define lifecycle management (creation, reset, cleanup)
   - Plan integration with existing method signatures

2. **Design External Interface**
   - Maintain compatibility with existing `chopBigTree()` method signature
   - Create adapter layer if necessary to preserve existing API
   - Plan parameter passing and result handling

3. **Error Handling Integration**
   - Map existing error conditions to state machine error events
   - Ensure consistent error reporting and recovery
   - Maintain existing logging and debugging capabilities

### Phase 4: Implementation
1. **Create State Machine Module**
   - Implement state machine in separate module for testing
   - Include comprehensive state transition definitions
   - Add debugging and introspection capabilities

2. **Implement Output Actions**
   - Move business logic from original method to output actions
   - Ensure actions receive necessary context and parameters
   - Maintain existing functionality exactly

3. **Create Integration Layer**
   - Modify `chopBigTree()` to use state machine
   - Handle state machine lifecycle within method
   - Ensure seamless integration with existing code

### Phase 5: Testing and Validation
1. **Unit Testing**
   - Test individual state transitions in isolation
   - Verify output actions execute correctly
   - Test error conditions and edge cases

2. **Integration Testing**
   - Test complete tree chopping cycles
   - Verify identical behavior to original implementation
   - Test interruption and recovery scenarios

3. **Performance Validation**
   - Ensure state machine overhead is acceptable
   - Profile memory usage and execution time
   - Validate real-world performance in Minecraft environment

### Phase 6: Migration and Cleanup
1. **Gradual Migration**
   - Deploy with feature flag if possible
   - Monitor behavior in production environment
...
- **Challenge**: The current implementation checks `self.stopActivity` at multiple points
- **Solution**: Ensure all states can handle STOP_ACTIVITY event gracefully
- **Consideration**: State machine should cleanly abort without leaving partial state

### Resource Management
- **Challenge**: Tool wielding (Stone Axe) and inventory management
- **Solution**: Ensure proper tool selection in output actions, handle tool unavailability
- **Consideration**: May need to add tool checking states or validation

### Position Recovery
- **Challenge**: If bot position is lost or incorrect, state machine may become confused
- **Solution**: Add position validation to state entry conditions
- **Consideration**: May need recovery mechanisms or position reset capabilities

### Performance Considerations
- **Challenge**: Current implementation has built-in delays (0.5-second sleeps)
- **Solution**: Handle timing within output actions, not state transitions
- **Consideration**: State machine overhead should not significantly impact performance

### Debugging and Observability
- **Challenge**: Current implementation has extensive debug logging
- **Solution**: Maintain debug capabilities through state machine introspection
- **Consideration**: State transitions should be observable for troubleshooting

### Tree Structure Variations
- **Challenge**: Real Minecraft trees may have irregular growth patterns
- **Solution**: Robust validation in VALIDATING_TREE state
- **Consideration**: May need more sophisticated tree analysis algorithms

### Network Latency and Timing
- **Challenge**: Minecraft network operations may have variable timing
- **Solution**: Build retry logic into output actions
- **Consideration**: State machine should be resilient to timing variations

### Memory and State Persistence
- **Challenge**: Long-running operations may need state persistence
- **Solution**: Design state machine to be serializable if needed
- **Consideration**: May need to handle state machine reconstruction after interruption

### Concurrent Operations
- **Challenge**: Other bot activities may interfere with tree chopping
- **Solution**: Proper coordination with other bot systems
- **Consideration**: State machine should coordinate with movement and inventory systems

### Error Recovery Strategies
- **Challenge**: Partial completion scenarios (tree half-chopped)
- **Solution**: Design recovery mechanisms for partial states
- **Consideration**: May need to detect and handle partially completed operations

## Conclusion

Converting the Wood Chopping State Machine from its current implicit implementation to an explicit Automat-based state machine will provide significant benefits in terms of code clarity, testability, and maintainability. The conversion requires careful attention to preserving existing functionality while making the state transitions explicit and observable.

The key success factors for this conversion are:
1. **Maintaining exact behavioral compatibility** with the existing implementation
2. **Preserving all error handling and edge case behavior**
3. **Ensuring robust interruption and recovery capabilities**
4. **Maintaining performance characteristics of the original code**
5. **Providing clear observability into state machine operation**

The conversion should be approached incrementally, with thorough testing at each phase to ensure the state machine behaves identically to the original implementation while providing the benefits of explicit state management.-e \n
# Farming Task State Machine Analysis and Conversion Plan

## Overview

The Farming Task State Machine is a sequential sub-machine that executes the logic for the `FARMING` state of the main bot activity state machine. This analysis examines the current implicit state management in `farming.py` and provides a detailed plan for converting it to an explicit Automat state machine.

## 1. Current State Analysis

### Implicit State Management Patterns

The farming state machine is currently hidden within the procedural flow of the `doFarming()` method. The states are managed implicitly through:

- **Program Counter Flow**: The state is determined by which section of the main `while not self.stopActivity` loop is currently executing
- **Sequential Execution**: States progress in a fixed order: Harvesting → Planting → Depositing → (Optional Break) → Repeat
- **Loop Counters**: The `break_interval` variable controls how many attempts are made in each state before moving to the next
- **Break Counter**: The `long_break` variable accumulates when no work is found, triggering extended rest periods
- **Activity Status**: The `self.stopActivity` flag can interrupt any state and terminate the entire farming cycle

### Current State Indicators

The current state is implicitly indicated by:
- The position within the main loop execution
- Activity status messages displayed via `self.refreshActivity()`
- Debug messages printed to console
- The specific work being performed (digging, planting, walking, sleeping)

## 2. State Identification

The farming system operates in four distinct behavioral states:

### HARVESTING State
- **Behavioral Characteristics**: Bot searches for and harvests mature crops
- **Operations Available**: Find harvestable blocks, walk to crops, dig crops
- **Success Condition**: Found and harvested a crop
- **Failure Condition**: No harvestable crops found within range
- **Resource Requirements**: None (harvesting generates resources)

### PLANTING State  
- **Behavioral Characteristics**: Bot searches for empty farmland and plants seeds
- **Operations Available**: Find empty soil, walk to location, place seeds
- **Success Condition**: Successfully planted seeds on empty farmland
- **Failure Condition**: No seeds available or no empty farmland found
- **Resource Requirements**: Seeds in inventory

### DEPOSITING State
- **Behavioral Characteristics**: Bot returns to base area and restocks from chest
- **Operations Available**: Walk to start position, transfer items to/from chest
- **Success Condition**: Inventory restocked according to equipment list
- **Failure Condition**: Cannot reach chest or insufficient supplies
- **Resource Requirements**: Access to chest with farming supplies

### RESTING State
- **Behavioral Characteristics**: Bot takes a break when no work is available
- **Operations Available**: Display status message, sleep for extended period
- **Success Condition**: Break period completed
- **Failure Condition**: None (always succeeds after timeout)
...
### External Events (Method Calls)
- **start_farming**: Initiates the farming cycle (implicit via doFarming() call)
- **stop_activity**: Terminates farming and returns to idle (via self.stopActivity flag)

### Internal Events (Condition-Based)
- **crops_harvested**: Triggered when harvestable crops are found and processed
- **no_crops_found**: Triggered when no harvestable crops remain within range
- **seeds_planted**: Triggered when seeds are successfully planted
- **no_planting_available**: Triggered when no seeds or no empty farmland available
- **restocking_complete**: Triggered when bot returns to base and completes inventory management
- **break_needed**: Triggered when long_break counter indicates extended rest is required
- **break_finished**: Triggered when sleep period completes
- **task_interrupted**: Triggered when stopActivity flag is set

### Loop Control Events
- **harvest_cycle_complete**: Triggered when break_interval iterations of harvesting complete
- **planting_cycle_complete**: Triggered when break_interval iterations of planting complete
...
**From HARVESTING:**
- **Event**: `no_crops_found` OR `harvest_cycle_complete` → **To**: PLANTING
- **Event**: `task_interrupted` → **To**: TERMINATED

**From PLANTING:**
- **Event**: `no_planting_available` OR `planting_cycle_complete` → **To**: DEPOSITING  
- **Event**: `task_interrupted` → **To**: TERMINATED

**From DEPOSITING:**
- **Event**: `restocking_complete` AND `break_needed` → **To**: RESTING
- **Event**: `restocking_complete` AND NOT `break_needed` → **To**: HARVESTING
- **Event**: `task_interrupted` → **To**: TERMINATED

**From RESTING:**
- **Event**: `break_finished` → **To**: HARVESTING
- **Event**: `task_interrupted` → **To**: TERMINATED

### Transition Conditions

**Break Decision Logic:**
- `break_needed` condition: `long_break >= 2` (both harvesting and planting found no work)
- `long_break` increments when no work is found in a state
- `long_break` resets to 0 at the start of each farming cycle

**Cycle Completion Logic:**
- Each work state (HARVESTING, PLANTING) can perform up to `break_interval` (32) iterations
- Early termination occurs when no work is found or task is interrupted

## 5. Output Identification

### State Entry Actions
- **HARVESTING**: Display "Harvesting" status, reset work counters
- **PLANTING**: Display "Planting" status, wield appropriate seeds
- **DEPOSITING**: Walk to start position, initiate restocking
- **RESTING**: Display "Taking a break" status

### Transition Actions
- **Finding Work**: Update activity display with specific item/location found
- **Work Completion**: Execute dig/place operations, update debug logs
- **No Work Found**: Update activity display with "no more X" message, increment long_break
- **Error Handling**: Log exceptions, continue to next iteration

### State Exit Actions
- **HARVESTING**: Calculate if any work was performed
- **PLANTING**: Check final seed inventory status
- **DEPOSITING**: Verify restocking completed successfully
- **RESTING**: Reset break counters

## 6. Conversion Plan

### Phase 1: State Machine Definition
1. **Create Automat State Machine Class**: Define `FarmingStateMachine` inheriting from appropriate Automat base
2. **Define State Enumeration**: Create explicit state constants for HARVESTING, PLANTING, DEPOSITING, RESTING, TERMINATED  
3. **Define Event Enumeration**: Create event constants for all identified transition triggers
4. **Create Initial State Configuration**: Set HARVESTING as the starting state

### Phase 2: Event Infrastructure
1. **Implement Internal Event Detection**: Create methods to evaluate transition conditions
   - `_check_harvest_complete()`: Evaluates crop availability and cycle limits
   - `_check_planting_complete()`: Evaluates seed/soil availability and cycle limits  
   - `_check_break_needed()`: Evaluates long_break counter
2. **Create Event Dispatch System**: Implement mechanism to trigger events based on work results
3. **Integrate Stop Signal**: Connect `self.stopActivity` to `task_interrupted` event

### Phase 3: Output Implementation
1. **Extract Work Logic**: Move harvesting/planting/depositing logic into separate output methods
2. **Create State Entry Handlers**: Implement `on_enter_*` methods for each state
3. **Create Transition Handlers**: Implement actions that occur during specific transitions
4. **Preserve Activity Display**: Maintain existing status update functionality within outputs

### Phase 4: Integration and Refactoring
1. **Replace Loop Structure**: Replace while loop with state machine event loop
2. **Maintain Public Interface**: Ensure `doFarming()` method signature remains unchanged
3. **Preserve Configuration**: Keep farming_blocks, farming_items, farming_seeds, and farmingEquipList unchanged
4. **Handle Area Management**: Integrate workArea functionality with state machine lifecycle

### Phase 5: Error Handling and Robustness
1. **Exception Management**: Ensure exceptions in outputs don't corrupt state machine state
2. **Invalid Transition Handling**: Define behavior for unexpected event/state combinations
3. **Recovery Mechanisms**: Implement fallback behaviors for error conditions
4. **State Persistence**: Consider whether state should survive bot restarts

### Phase 6: Testing and Validation
1. **Behavioral Equivalence Testing**: Verify converted code produces identical farming patterns
2. **State Coverage Testing**: Ensure all states and transitions are exercised
3. **Performance Validation**: Confirm no significant performance degradation
4. **Long-Running Stability**: Test extended farming sessions for state machine stability

## 7. Edge Cases and Special Considerations

### Resource Depletion Scenarios
- **Seed Shortage**: When planting finds no seeds, system should gracefully skip planting state
- **Storage Full**: When depositing cannot store items, consider how this affects farming continuation
- **Tool Durability**: If farming tools break, state machine should handle tool replacement or termination

### Timing and Synchronization
- **Sleep Integration**: The current 30-second sleep in RESTING state must not block event processing
- **Activity Refresh**: Status updates must continue to work with async state machine execution  
- **Stop Signal Timing**: Ensure `self.stopActivity` is checked promptly in all states

### Environmental Changes
- **Work Area Validity**: Handle cases where work area becomes invalid during farming
- **Chest Accessibility**: Manage scenarios where chest becomes unreachable
- **Crop Growth Timing**: Account for natural crop growth cycles affecting available work

### Concurrent Operations
- **Survival State Machine**: Ensure farming state machine can be interrupted by health/hunger management
- **Main Activity Coordination**: Maintain proper interaction with higher-level activity state machine
- **Chat Command Integration**: Preserve ability for users to stop farming via chat commands

### Migration Strategy
- **Incremental Deployment**: Plan for running old and new implementations in parallel during validation
- **Configuration Compatibility**: Ensure existing farming configurations continue to work
- **Debug Information**: Maintain or enhance current logging and debug output
- **Performance Monitoring**: Track any changes in farming efficiency or resource usage

### State Machine Architecture Decisions
- **Single vs Multiple Machines**: Consider whether break logic should be a separate state machine
- **Event Granularity**: Balance between too many fine-grained events and too few coarse ones
- **State Granularity**: Determine if sub-states are needed (e.g., separate states for searching vs performing work)
- **Output Composition**: Decide how to structure outputs for maximum reusability and clarity

## Summary

The Farming Task State Machine conversion represents a moderately complex transformation from implicit sequential logic to explicit state management. The current implementation's strength lies in its straightforward sequential flow, but this simplicity masks important state-dependent behaviors and transition conditions.

The conversion to Automat will expose these hidden patterns, making the farming logic more maintainable, testable, and extensible. The key challenge lies in preserving the existing timing and coordination behaviors while gaining the benefits of explicit state management.

Success will be measured by achieving behavioral equivalence while improving code clarity and enabling future enhancements to farming logic without disrupting the overall bot architecture.-e \n
# Chest Interaction State Machine Analysis and Conversion Plan

## Overview

This document provides a detailed analysis of the implicit Chest Interaction State Machine found in the `inventory.py` file within the `Chest` class, and outlines a comprehensive plan for converting it to an explicit state machine using the Automat library.

## 1. Current State Analysis

### Implicit State Management

The current implementation manages state implicitly through the `self.container` attribute:
- **State Variable**: `self.container` serves as the primary state indicator
  - `None` value indicates the chest is in a CLOSED state
  - Non-None value (containing a chest container object) indicates the chest is in an OPEN state

### Hidden State Behaviors

The code exhibits several patterns that reveal implicit state dependencies:

1. **Guard Conditions**: The `open()` method contains a guard condition `if self.container: return True` that prevents re-opening an already open chest
2. **State Assumptions**: Methods like `itemCount()`, `depositItem()`, and `withdrawItem()` assume the chest is already open and operate directly on `self.container`
3. **Auto-State Transitions**: Some methods like `spaceAvailable()` and `printContents()` automatically transition to the OPEN state by calling `open()` internally
4. **Container Mutation**: The container object itself can change during operations (deposit/withdraw operations may return new container objects)

### Behavioral Inconsistencies

The current implementation shows inconsistent state handling:
- Some methods explicitly call `open()` before proceeding
- Other methods assume the chest is already open
- The `close()` method lacks proper state validation
- Error handling is inconsistent across state-dependent operations

## 2. State Identification

Based on analysis of the code behavior and method dependencies, the following distinct states have been identified:

### Primary States

1. **UNINITIALIZED**
   - Initial state when the Chest object is created but the physical chest object hasn't been located
   - Occurs when `self.object` is `None` during initialization
   - No operations are valid in this state

2. **CLOSED**
   - The chest object has been located but no container connection exists
   - `self.container` is `None` and `self.object` is not `None`
   - Limited operations are available (only opening operations)

3. **OPEN**
   - Active container connection exists
   - `self.container` contains a valid chest container object
   - All inventory operations are available

4. **ERROR**
   - State entered when operations fail or when chest becomes inaccessible
   - Represents failure conditions that require external intervention
   - Should prevent further operations until resolved

### State Characteristics

- **UNINITIALIZED**: No operations permitted except re-initialization
- **CLOSED**: Only opening operations permitted
- **OPEN**: All inventory operations (read/write) permitted
- **ERROR**: No operations permitted until error is resolved
...
1. **initialize**
   - Triggered by object construction or re-initialization attempts
   - Searches for and establishes connection to physical chest object
   - Can succeed or fail based on chest availability

2. **open**
   - Explicit request to open the chest
   - Establishes container connection
   - Can fail if chest is inaccessible or already open

3. **close**
   - Explicit request to close the chest
   - Terminates container connection
   - Should validate that chest is actually open

4. **read_operation**
   - Represents read-only operations: `spaceAvailable()`, `printContents()`, `itemCount()`
   - Should automatically open chest if needed
   - Should not change fundamental state

5. **write_operation**
   - Represents write operations: `depositItem()`, `withdrawItem()`, `deposit()`, `restock()`
   - Requires chest to be open
   - May modify container object during operation

6. **error_occurred**
   - Internal event triggered when operations fail
   - Network errors, chest disappearance, permission issues
   - Should transition to error state for proper handling

### Internal Events

1. **chest_found**
   - Internal event when chest object is successfully located
   - Transitions from UNINITIALIZED to CLOSED

2. **chest_not_found**
   - Internal event when chest cannot be located
   - Keeps system in UNINITIALIZED or transitions to ERROR

3. **container_established**
   - Internal event when container connection succeeds
   - Transitions from CLOSED to OPEN

4. **container_lost**
   - Internal event when container connection fails or is lost
   - Transitions from OPEN to CLOSED or ERROR
...
#### From UNINITIALIZED
- **initialize + chest_found** → CLOSED
- **initialize + chest_not_found** → ERROR

#### From CLOSED  
- **open + container_established** → OPEN
- **open + container_lost** → ERROR
- **read_operation** → OPEN (auto-open behavior)
- **write_operation** → OPEN (auto-open behavior)

#### From OPEN
- **close** → CLOSED
- **read_operation** → OPEN (self-transition)
- **write_operation** → OPEN (self-transition, may update container)
- **error_occurred** → ERROR
- **container_lost** → CLOSED

#### From ERROR
- **initialize** → CLOSED (recovery attempt)

### Invalid Transitions

- **UNINITIALIZED** → **OPEN** (cannot open without locating chest first)
- **ERROR** → **OPEN** (must recover through re-initialization)
- **ERROR** → **CLOSED** (must recover through re-initialization)

### Guard Conditions

1. **Container Already Open**: `open()` should return immediately if already open
2. **Container Not Open**: `close()` should validate chest is actually open
3. **Chest Object Exists**: Operations require valid `self.object` 
4. **Container Still Valid**: Long-running operations should verify container hasn't been invalidated
...
1. **Entering CLOSED**
   ...
   - Validate `self.object` is not None

2. **Entering OPEN**
   - Establish container connection via `self.pybot.bot.openContainer()`
   - Add connection delay (`time.sleep(0.2)`)
   - Validate successful connection
   - Update `self.container` with new container object

3. **Entering ERROR**
   - Log error conditions
   - Set both `self.container` and potentially `self.object` to None
   - Preserve error information for recovery

### Transition Actions

1. **Opening Operations**
   - Call `self.pybot.bot.openContainer(self.object)`
   - Handle network timeouts and connection failures
   - Update container reference if successful
...
3. **Container Updates**
   - Some operations (deposit/withdraw) return new container objects
   - Update `self.container` reference when this occurs
   - Maintain consistency of container state

### Side Effects

1. **Logging and Debug Output**
   - State transitions should be logged for debugging
   - Operation results should be reported appropriately
   - Error conditions should be clearly documented

2. **Error Reporting**
   - Failed operations should use `self.pybot.perror()` consistently
   - Exception handling should preserve original error information
   - Recovery suggestions should be provided where appropriate

## 6. Conversion Plan

### Phase 1: State Machine Definition

1. **Define Automat State Machine**
   - Create `ChestStateMachine` class inheriting from appropriate Automat base
   - Define four states: UNINITIALIZED, CLOSED, OPEN, ERROR
   - Set UNINITIALIZED as initial state

2. **Define Input Events**
   - `initialize(chest_type, silent)` - for object initialization
   - `open()` - explicit open request
   - `close()` - explicit close request  
   - `read_operation(operation_name, *args)` - for read operations
   - `write_operation(operation_name, *args)` - for write operations
   - `error_occurred(error_info)` - for error handling

3. **Define Output Actions**
   - `setup_chest_object()` - locate and validate chest
   - `establish_container()` - create container connection
   - `cleanup_container()` - close and cleanup container
   - `handle_error()` - error state management
   - `execute_read()` - perform read operations
   - `execute_write()` - perform write operations

### Phase 2: State Transition Implementation

1. **Implement State Entry/Exit Actions**
   - UNINITIALIZED entry: clear all object references
   - CLOSED entry: ensure container is None, object is valid
   - OPEN entry: establish and validate container connection  
   - ERROR entry: log error and clean up inconsistent state

2. **Implement Transition Logic**
   - Add guard conditions for all transitions
   - Implement proper error handling for failed transitions
   - Add automatic recovery mechanisms where appropriate

3. **Integrate Container Management**
   - Handle container object updates during operations
   - Maintain consistency between state and container status
   - Implement proper cleanup on state transitions

### Phase 3: Method Integration

1. **Convert Existing Methods**
   - `open()` becomes event trigger to state machine
   - `close()` becomes event trigger to state machine
   - `spaceAvailable()`, `printContents()`, `itemCount()` become read operations
   - `depositItem()`, `withdrawItem()`, `deposit()`, `restock()` become write operations

2. **Preserve Public Interface**
   - Maintain existing method signatures
   - Ensure backward compatibility with existing callers
   - Add deprecation warnings if interface changes are needed

3. **Add State Validation**
   - All public methods should validate appropriate state before proceeding
   - Clear error messages for operations attempted in wrong state
   - Automatic state correction where safe and appropriate

### Phase 4: Error Handling Enhancement

1. **Comprehensive Error Handling**
   - Catch and classify all possible error conditions
...
3. **State Consistency Validation**
   ...
   - Implement state repair mechanisms where possible
...
2. **Integration Testing**
   - Test with existing bot infrastructure
   - Verify performance characteristics
   - Test error recovery under real network conditions
...
**Issue**: Some operations (deposit/withdraw) may return new container objects, requiring the state machine to update its internal references while remaining in the OPEN state.

**Solution**: Implement self-transitions in the OPEN state that update the container reference. Add validation to ensure the new container is valid and represents the same physical chest.

### Network Timeouts and Disconnections

**Issue**: Minecraft server connections can timeout or disconnect, invalidating container objects without explicit notification.

**Solution**: Implement periodic container validation and automatic error detection. Add retry logic with exponential backoff. Provide clear recovery paths from ERROR state.

### Concurrent Access Patterns

**Issue**: Multiple parts of the bot may attempt to access the same chest simultaneously, leading to race conditions.

**Solution**: Consider implementing chest locking mechanisms or request queuing. Document thread safety characteristics. Add detection for concurrent access attempts.

### Chest Type Variations

**Issue**: The current code handles both regular chests and minecart chests with different access patterns.

**Solution**: Make chest type handling explicit in the state machine. Consider separate state machines for different chest types if behavior diverges significantly.

### Silent Operation Mode

**Issue**: The constructor accepts a `silent` parameter that suppresses error messages, complicating error handling.

**Solution**: Make silent mode a property of the state machine rather than a constructor parameter. Allow error reporting to be configured independently of error handling.

### Resource Cleanup

**Issue**: Container objects may hold network resources that need proper cleanup even in error conditions.

**Solution**: Implement proper resource cleanup in all state exit actions. Add finalizer methods to ensure cleanup occurs even if close() is not called explicitly.

### State Persistence

**Issue**: Long-running operations may need to survive bot restarts or temporary disconnections.

**Solution**: Consider adding state serialization capabilities. Design recovery mechanisms that can restore appropriate state after reconnection.

## Implementation Notes

### Automat Integration

- Use `@Automat.input` decorators for event methods
- Use `@Automat.output` decorators for action methods  
- Leverage Automat's transition validation and visualization capabilities
- Consider using Automat's debugging and introspection features
...
- Add migration path for code that depends on internal state

### Performance Considerations

- State machine overhead should be minimal compared to network operations
- Consider caching frequently accessed state information
- Optimize for common usage patterns (open → multiple operations → close)
- Profile state machine performance under realistic load

### Documentation and Maintenance

- Generate state diagrams from Automat definitions
- Document all states, events, and transitions thoroughly
- Provide examples of proper usage patterns
- Create troubleshooting guide for common error conditions

This conversion plan provides a comprehensive roadmap for transforming the implicit Chest Interaction State Machine into an explicit, robust, and maintainable implementation using the Automat library.-e \n
# Survival State Machine Conversion Plan

## Overview
This document provides a detailed natural language analysis and conversion plan for transforming the implicit Survival State Machine in the Minecraft bot's combat.py module into an explicit state machine using the Automat library.

## 1. Current State Analysis

### Implicit State Management
The current implementation uses a hidden state machine that manages the bot's survival behavior through a combination of:
- **Boolean flag**: `self.healMode` serves as the primary state indicator
- **Conditional logic**: Scattered throughout `healthCheck()` and `healToFull()` methods
- **Side effects**: Direct manipulation of `self.stopActivity` and `self.dangerType` variables

### Behavioral Patterns
The system exhibits two distinct behavioral modes:
1. **Active monitoring mode**: Continuously checking health/food levels and responding to threats
2. **Passive recovery mode**: Waiting for health and food to restore while suspended from other activities

The state transitions are triggered by health/food thresholds and recovery completion, but the logic is distributed across multiple methods without clear state boundaries.

### Hidden State Indicators
- `self.healMode = True/False` acts as the primary state variable
- Method behavior changes based on this flag (e.g., `healthCheck()` returns early if in heal mode)
- The state affects the bot's ability to perform other activities
- Recovery loop in `healToFull()` represents a blocking state transition
...
- **Condition**: `healMode = False`
- **Behavior**: Actively monitors health and food levels
- **Capabilities**: Can trigger emergency responses, set danger flags, force activity stops
- **Characteristics**: Responsive to health/food changes, can interrupt main activities

**RECOVERING (Healing Mode)**
- **Condition**: `healMode = True` 
- **Behavior**: Passively waits for health/food restoration
- **Capabilities**: Attempts to eat food, blocks other activities
- **Characteristics**: Blocking behavior, focused solely on recovery

### State Characteristics

**MONITORING State Properties:**
- Performs health checks (health <= 90% triggers activity stop)
- Performs food checks (food <= 50% triggers activity stop)
- Can trigger panic quit (health <= 50%)
- Sets danger type indicators
- Manages stopActivity flag

**RECOVERING State Properties:**
- Blocks until health >= 20/20 (100%) AND food >= 18/20 (90%)
- Attempts to eat food if available
- Provides periodic status updates
- Prevents other activities from running
...
**startHealing**
- **Trigger**: Explicit call to `healToFull()`
- **Preconditions**: Bot health < 20 OR food < 18
- **Purpose**: Initiate recovery process
- **Current Implementation**: Direct method call

**healthCheck**
- **Trigger**: Periodic calls from main activity loop
- **Preconditions**: None (always callable)
- **Purpose**: Monitor current health/food status
- **Current Implementation**: Returns boolean, sets side effects

### Internal Events (Condition-Based)

**healthRestored**
- **Trigger**: Health reaches 20/20 AND food reaches 18/20
- **Source**: Monitoring loop within `healToFull()`
- **Purpose**: Signal recovery completion
- **Current Implementation**: Loop exit condition

**healthCritical**
- **Trigger**: Health drops to 50% or below
- **Source**: Threshold check in `healthCheck()`
- **Purpose**: Trigger emergency quit
- **Current Implementation**: Immediate system exit

**healthLow**
- **Trigger**: Health drops to 90% or below
- **Source**: Threshold check in `healthCheck()`
- **Purpose**: Signal need for recovery
- **Current Implementation**: Sets stopActivity flag

**foodLow**
- **Trigger**: Food drops to 50% or below
- **Source**: Threshold check in `healthCheck()`
- **Purpose**: Signal need for feeding
- **Current Implementation**: Sets stopActivity flag
...
- **Event**: `startHealing` (explicit) OR `healthLow`/`foodLow` (implicit)
- **Conditions**: Health < 20 OR food < 18
- **Actions**: Set healMode = True, begin recovery loop
- **Current Implementation**: `healToFull()` method call

**RECOVERING → MONITORING**
- **Event**: `healthRestored` (internal)
- **Conditions**: Health >= 20 AND food >= 18
- **Actions**: Set healMode = False, exit recovery loop
- **Current Implementation**: Loop exit in `healToFull()`

### Special Transitions

**MONITORING → SYSTEM_EXIT**
- **Event**: `healthCritical`
- **Conditions**: Health <= 50%
- **Actions**: Emergency quit with system exit
- **Current Implementation**: Direct `sys.exit()` call

### Self-Transitions (Same State)

**MONITORING → MONITORING**
- **Event**: `healthCheck` (periodic)
- **Conditions**: Health and food within acceptable ranges
- **Actions**: Update status, continue monitoring
- **Current Implementation**: Method return without state change

**RECOVERING → RECOVERING**
- **Event**: Recovery attempt (internal)
- **Conditions**: Health < 20 OR food < 18
- **Actions**: Attempt to eat, wait, continue recovery
- **Current Implementation**: Loop continuation in `healToFull()`

## 5. Output Identification

### Monitoring State Outputs

**logHealthStatus**
- **Purpose**: Report current health/food percentages
- **Current Implementation**: `self.pdebug()` calls with health/food info
- **Trigger**: Every health check cycle

**setDangerFlag**
- **Purpose**: Indicate type of danger detected
- **Current Implementation**: `self.dangerType = "danger: health"` or `"out of food"`
- **Trigger**: Health/food threshold breaches

**stopCurrentActivity**
- **Purpose**: Interrupt main activity due to survival needs
- **Current Implementation**: `self.stopActivity = True`
- **Trigger**: Health <= 90% or food <= 50%

**emergencyQuit**
- **Purpose**: Immediate system shutdown for critical health
- **Current Implementation**: `self.bot.end()` + `sys.exit()`
- **Trigger**: Health <= 50%

### Recovery State Outputs

**logRecoveryStart**
- **Purpose**: Announce beginning of recovery process
- **Current Implementation**: "Bot is injured or hungry, resting." message
- **Trigger**: Entry into recovery state

**attemptEating**
- **Purpose**: Try to consume food for healing
- **Current Implementation**: `self.eatFood()` call
- **Trigger**: Each recovery loop iteration

**logRecoveryProgress**
- **Purpose**: Report health/food restoration progress
- **Current Implementation**: Periodic health/food percentage updates
- **Trigger**: When health increases during recovery

**logRecoveryComplete**
- **Purpose**: Announce successful recovery
- **Current Implementation**: "done." message with final stats
- **Trigger**: Exit from recovery state

## 6. Conversion Plan

### Phase 1: State Machine Design
1. **Define Automat states**: Create `MONITORING` and `RECOVERING` states
2. **Define events**: Create `startHealing`, `healthRestored`, `healthCritical`, `healthLow`, `foodLow`, `healthCheck` events
3. **Map transitions**: Create transition table connecting states via events
4. **Design outputs**: Create output methods for each side effect identified above

### Phase 2: Automat Implementation
1. **Create state machine class**: Inherit from or compose with existing CombatBot
2. **Implement state methods**: Create `@state` decorated methods for each state
3. **Implement input methods**: Create `@input` decorated methods for each event
4. **Implement output methods**: Create `@output` decorated methods for each action
5. **Define transition table**: Use Automat's transition syntax to connect states, events, and outputs

### Phase 3: Integration
1. **Preserve existing interface**: Maintain `healthCheck()` and `healToFull()` method signatures
2. **Replace boolean flag**: Remove `healMode` variable, use state machine state instead
3. **Refactor condition logic**: Move threshold checks into appropriate input methods
4. **Update calling code**: Ensure other modules can still call existing methods

### Phase 4: Enhanced Behavior
1. **Add state introspection**: Provide methods to query current state
2. **Improve error handling**: Add proper exception handling for failed transitions
3. **Add logging**: Enhance debug output with state transition information
4. **Add validation**: Ensure state transitions are valid and logged

### Phase 5: Testing and Validation
1. **Unit tests**: Test each state, event, and transition independently
2. **Integration tests**: Verify behavior matches original implementation
3. **Edge case testing**: Test boundary conditions and error scenarios
4. **Performance testing**: Ensure state machine doesn't add significant overhead

## 7. Edge Cases and Special Considerations

### Critical Health Handling
- **Issue**: Current implementation uses immediate `sys.exit()` for critical health
- **Consideration**: This bypasses normal state machine flow
- **Solution**: Either maintain direct exit or create a special `CRITICAL` state that handles emergency shutdown

### Concurrent Access
- **Issue**: Health checks may be called while recovery is in progress
- **Consideration**: State machine should handle concurrent access safely
- **Solution**: Ensure state machine operations are atomic or use appropriate locking

### Recovery Interruption
- **Issue**: Current recovery loop blocks indefinitely
- **Consideration**: Need ability to interrupt recovery if conditions change
- **Solution**: Add `interruptRecovery` event or make recovery loop non-blocking

### Food Availability
- **Issue**: Recovery can fail if no food is available
- **Consideration**: `eatFood()` failure should be handled gracefully
- **Solution**: Add `foodUnavailable` event that transitions to appropriate state

### State Persistence
- **Issue**: Bot state may need to persist across restarts
- **Consideration**: Current implementation doesn't handle state persistence
- **Solution**: Consider adding state serialization/deserialization if needed

### Integration with Main Activity
- **Issue**: Survival state machine interacts with main activity state machine
- **Consideration**: Need clear communication protocol between state machines
- **Solution**: Maintain existing `stopActivity` flag mechanism or create formal inter-machine communication

### Performance Optimization
- **Issue**: Frequent health checks may impact performance
- **Consideration**: State machine overhead should be minimal
- **Solution**: Optimize state machine implementation and consider check frequency

### Error Recovery
- **Issue**: What happens if state machine gets into invalid state
- **Consideration**: Need robust error handling and recovery mechanisms
- **Solution**: Add state validation and error recovery transitions

This conversion plan provides a comprehensive roadmap for transforming the implicit survival state machine into an explicit, maintainable, and extensible Automat-based implementation while preserving all existing functionality and improving system clarity.-e \n
# Building Task State Machine Analysis and Conversion Plan

## Overview

This document provides a detailed natural language plan for converting the Building Task State Machine from its current implicit implementation in `build.py` to an explicit state machine using the Automat library. The building system manages multi-phase construction of blueprints with complex block placement logic.

## 1. Current State Analysis

### Implicit State Management

The building state machine is currently implemented as a hidden state machine within the `doBuild()` method in `/Users/ohadr/goap/mountain-goap-python/minebot/build.py`. The state is managed through several mechanisms:

- **Primary State Variable**: The `stage` integer variable (line 83) explicitly tracks the current phase of construction, starting at 1 and incrementing after each phase completes
- **Loop-Based Control**: The main `while not self.stopActivity` loop (line 85) controls the overall execution flow
- **Nested Control Structures**: Inner loops for z, y, x coordinates (lines 138-140) manage block placement within each phase
- **Conditional Logic**: Various if/elif statements determine the current operational mode within each phase

### Current Behavioral Patterns

The system exhibits distinct behavioral patterns based on the stage variable and execution context:

1. **Blueprint Loading**: Attempts to load blueprint for current stage using naming convention `{blueprint_name}_{stage_number}`
2. **Area Definition**: Creates work area and validates it for the current phase
3. **Analysis Phase**: Examines existing blocks versus blueprint requirements, calculating needed materials
4. **Restocking Phase**: Calls `area.restock(need)` to gather required materials
5. **Construction Phase**: Systematically places blocks following back-to-front, bottom-to-top order
6. **Phase Completion**: Increments stage and repeats until no more phases exist

### Problems with Current Implementation

- **Lack of Explicit State Transitions**: State changes happen implicitly through loop progression and variable modification
- **Error Handling Scattered**: Error conditions can terminate the process at multiple points without clear recovery paths
- **No State Persistence**: If interrupted, the system cannot resume from the exact point of failure
- **Difficult Testing**: Cannot easily test individual states or transitions in isolation
- **Hidden Dependencies**: State dependencies are implicit in the code flow rather than explicitly declared

## 2. State Identification

### Primary States

Based on the code analysis, the building task state machine has the following distinct states:

**BLUEPRINT_LOADING**
- The system attempts to load the blueprint for the current phase
- Validates blueprint existence and handles first-phase special cases
- Handles blueprint not found conditions differently for stage 1 vs. subsequent stages

**AREA_VALIDATION**
- Creates and validates the work area for the current phase
- Ensures the area dimensions match the blueprint requirements
- Can fail if the area is invalid, terminating the entire process

**ANALYSIS**
- Examines each block position in the work area
- Compares existing blocks with blueprint requirements
- Detects conflicts (wrong blocks already placed)
- Calculates material requirements for the phase

**RESTOCKING**
- Gathers required materials identified during analysis
- Always includes 2 bread for sustenance
- Delegates to area.restock() method
- Can be skipped if no materials are needed

**CONSTRUCTION**
- Systematically places blocks according to blueprint
- Manages complex placement logic including positioning, orientation, and special behaviors
- Handles sneak and jump states for special block types
- Can be interrupted by stopActivity or errors

**PHASE_COMPLETION**
- Increments stage counter
- Returns bot to area start position
- Prepares for next phase or termination

**TERMINATION**
- Handles normal completion (no more phases)
- Handles error conditions
- Calls endActivity() to clean up

### State Cohesion Analysis

Each identified state has clear behavioral characteristics:
- **BLUEPRINT_LOADING**: File system operations and blueprint validation
- **AREA_VALIDATION**: Spatial calculations and area setup
- **ANALYSIS**: Block comparison and material calculation
- **RESTOCKING**: Inventory management operations
- **CONSTRUCTION**: Block placement and bot movement
- **PHASE_COMPLETION**: Cleanup and progression logic
- **TERMINATION**: Final cleanup and state reset
...
**START_BUILD**
- Triggered by calling `doBuild(args)` method
- Carries blueprint name parameter
- Initiates the entire building process

**STOP_ACTIVITY**
- Triggered by setting `self.stopActivity = True`
- Can occur at any time during construction
- Forces immediate termination of current operations

### Internal Events

**BLUEPRINT_FOUND**
- Occurs when `getBlueprint(bp_name+"_"+str(stage))` returns a valid blueprint
- Triggers progression from BLUEPRINT_LOADING to AREA_VALIDATION

**BLUEPRINT_NOT_FOUND**
- Occurs when blueprint lookup fails
- Behavior depends on stage (error for stage 1, completion for later stages)

**AREA_VALID**
- Occurs when `area.valid` is True
- Triggers progression from AREA_VALIDATION to ANALYSIS

**AREA_INVALID**
- Occurs when work area validation fails
- Triggers immediate termination

**ANALYSIS_COMPLETE**
- Occurs when all blocks have been analyzed
- Triggers progression to RESTOCKING or CONSTRUCTION

**CONFLICT_DETECTED**
- Occurs when existing block doesn't match blueprint and isn't empty
- Triggers immediate termination with error message

**RESTOCKING_COMPLETE**
- Occurs when all required materials have been gathered
- Triggers progression to CONSTRUCTION

**BLOCK_PLACEMENT_SUCCESS**
- Occurs when individual block is successfully placed
- Continues construction loop

**BLOCK_PLACEMENT_FAILURE**
- Occurs when block placement fails
- Triggers immediate termination

**PHASE_COMPLETE**
- Occurs when all blocks for current phase have been placed
- Triggers progression to PHASE_COMPLETION

**NO_MORE_PHASES**
- Occurs when no blueprint exists for next stage
- Triggers normal termination

### Event Parameter Considerations

- **START_BUILD**: Carries blueprint name and any additional arguments
- **BLUEPRINT_FOUND**: Carries blueprint object and stage number
- **CONFLICT_DETECTED**: Carries position, expected block, and actual block information
- **BLOCK_PLACEMENT_FAILURE**: Carries block type, position, and failure reason

## 4. Transition Mapping

### Valid Transitions

**From BLUEPRINT_LOADING:**
- **→ AREA_VALIDATION** (on BLUEPRINT_FOUND)
- **→ TERMINATION** (on BLUEPRINT_NOT_FOUND for stage 1, or STOP_ACTIVITY)
- **→ TERMINATION** (on BLUEPRINT_NOT_FOUND for stage > 1, normal completion)

**From AREA_VALIDATION:**
- **→ ANALYSIS** (on AREA_VALID)
- **→ TERMINATION** (on AREA_INVALID or STOP_ACTIVITY)

**From ANALYSIS:**
- **→ RESTOCKING** (on ANALYSIS_COMPLETE with materials needed)
- **→ CONSTRUCTION** (on ANALYSIS_COMPLETE with no materials needed)
- **→ TERMINATION** (on CONFLICT_DETECTED or STOP_ACTIVITY)

**From RESTOCKING:**
- **→ CONSTRUCTION** (on RESTOCKING_COMPLETE)
- **→ TERMINATION** (on STOP_ACTIVITY)

**From CONSTRUCTION:**
- **→ PHASE_COMPLETION** (on PHASE_COMPLETE)
- **→ TERMINATION** (on BLOCK_PLACEMENT_FAILURE or STOP_ACTIVITY)

**From PHASE_COMPLETION:**
- **→ BLUEPRINT_LOADING** (on phase increment, loops back to start next phase)
- **→ TERMINATION** (on STOP_ACTIVITY)
...
Several transitions require guard conditions:

- **BLUEPRINT_LOADING → TERMINATION**: Requires `stage == 1` for error vs. completion
- **ANALYSIS → RESTOCKING**: Requires `need` dictionary to be non-empty
- **ANALYSIS → CONSTRUCTION**: Requires `need` dictionary to be empty
- **All states → TERMINATION**: Can occur when `stopActivity` is True

### Self-Transitions

- **CONSTRUCTION → CONSTRUCTION**: Occurs during block placement loop, doesn't change state but continues placing blocks
- **BLUEPRINT_LOADING → BLUEPRINT_LOADING**: Could occur if blueprint loading needs retry logic

## 5. Output Identification

### State Entry Outputs

**BLUEPRINT_LOADING Entry:**
- Print phase start message (for stage > 1)
- Load blueprint using naming convention

**AREA_VALIDATION Entry:**
- Create workArea object with blueprint dimensions
- Validate area dimensions and placement

**ANALYSIS Entry:**
- Initialize need dictionary with bread requirement
- Examine each block position in work area
- Compare existing vs. required blocks

**RESTOCKING Entry:**
- Print material requirements
- Call area.restock() method
- Print confirmation message

**CONSTRUCTION Entry:**
- Initialize control state variables (sneak, jump)
- Begin block placement loops

**PHASE_COMPLETION Entry:**
- Increment stage counter
- Move bot to area start position
- Add delay for positioning

**TERMINATION Entry:**
- Call endActivity() method
- Print completion or error messages

### Transition Outputs

**On BLUEPRINT_FOUND:**
- Print phase start message (stage > 1)

**On BLUEPRINT_NOT_FOUND:**
- Print error message (stage 1) or completion message

**On AREA_INVALID:**
- Print error message about area validation

**On CONFLICT_DETECTED:**
- Print detailed error message with position and block information

**On BLOCK_PLACEMENT_SUCCESS:**
- Print placement confirmation with coordinates and block type
- Update bot position and control states

**On BLOCK_PLACEMENT_FAILURE:**
- Print error message with block type and position
- Set stopActivity flag

### Complex Outputs

**Block Placement Sequence:**
1. Calculate bot position and target position
2. Determine placement strategy (top, front, special)
3. Move bot to correct position
4. Wield appropriate item
5. Set control states (sneak, jump) if needed
6. Execute block placement
7. Clear control states
8. Verify placement success

**Error Recovery Outputs:**
- Set stopActivity flag to halt all operations
- Print diagnostic information
- Ensure bot control states are reset

## 6. Conversion Plan

### Phase 1: Foundation Setup

**Step 1: Create Automat State Machine Class**
- Define BuildingStateMachine class inheriting from Automat's machine
- Declare all states as class attributes using @state decorator
- Import necessary Automat decorators and utilities

**Step 2: Define Input Methods**
- Create @input decorated methods for each external event
- startBuild(blueprint_name) for START_BUILD event
- stopActivity() for STOP_ACTIVITY event
- Ensure method signatures match existing public interface

**Step 3: Implement Internal Event Triggers**
- Create private methods for each internal event
- _blueprintFound(blueprint, stage) for BLUEPRINT_FOUND
- _blueprintNotFound(stage) for BLUEPRINT_NOT_FOUND
- _areaValid(area) for AREA_VALID
- _areaInvalid() for AREA_INVALID
- And so on for all internal events

### Phase 2: State Implementation

**Step 4: Implement State Methods**
- Create methods for each state's behavior
- _on_blueprint_loading() for BLUEPRINT_LOADING state logic
- _on_area_validation() for AREA_VALIDATION state logic
- Extract current logic from doBuild() method into appropriate state methods
- Ensure each state method is focused and cohesive

**Step 5: Implement Output Methods**
- Create separate methods for each identified output
- _load_blueprint(name, stage) for blueprint loading operations
- _validate_area(blueprint) for area validation operations
- _analyze_requirements(area, blueprint) for analysis operations
- _restock_materials(requirements) for restocking operations
- _place_block(position, block_type, area, blueprint) for construction operations

### Phase 3: Transition Logic

**Step 6: Define Transitions**
- Use @transition decorator for each valid transition
- Specify source state, target state, and trigger event
- Implement guard conditions using outputs parameter
- Handle conditional transitions with appropriate guard logic

**Step 7: Implement Guard Conditions**
- Create guard methods for conditional transitions
- _is_stage_one() for stage-dependent transitions
- _has_materials_needed() for restocking decision
- _has_more_phases() for phase completion logic
- Ensure guards return boolean values

### Phase 4: Integration and Testing

**Step 8: Integrate with Existing Code**
- Replace doBuild() method with state machine integration
- Maintain existing public interface for backward compatibility
- Ensure all existing functionality is preserved
- Handle state machine initialization and lifecycle

**Step 9: Add State Persistence**
- Implement state serialization for interruption recovery
- Add methods to save/restore current state and progress
- Enable resumption from specific phases or block positions
- Consider checkpoint creation at phase boundaries

**Step 10: Error Handling Enhancement**
- Implement robust error handling within state machine
- Add error recovery transitions where appropriate
- Ensure proper cleanup on all error conditions
- Add detailed error reporting and logging

### Phase 5: Testing and Validation

**Step 11: Unit Testing**
- Create tests for each state in isolation
- Test all valid transitions and their outputs
- Test invalid transitions and guard conditions
- Verify error handling and edge cases

**Step 12: Integration Testing**
- Test complete building scenarios with multiple phases
- Test interruption and resumption scenarios
- Verify backward compatibility with existing code
- Performance testing to ensure no significant overhead

**Step 13: Documentation and Visualization**
- Generate state diagrams from Automat machine definition
- Document all states, events, and transitions
...
**Multi-Phase Blueprint Handling:**
- Blueprints follow naming convention: {name}_{stage}
- Stage 1 must exist (error if not found)
- Subsequent stages are optional (completion if not found)
- Each phase may have different dimensions and requirements

**Blueprint Validation:**
- Ensure blueprint dimensions match work area capabilities
- Validate block types are supported and available
- Handle blueprint format errors gracefully

### Block Placement Complexity

**Placement Strategy Selection:**
- Complex logic determines how to place each block
- Multiple fallback strategies (top, front, special positioning)
- Custom placement specifications from blueprint build functions
- Interactive block handling (sneak required for chests, etc.)

**Bot Positioning and Movement:**
- Precise positioning required for block placement
- Safe movement to avoid falling or getting stuck
- Coordinate system transformation between blueprint and world
- Handling of jump and sneak states during placement

### Error Recovery and Robustness

**Conflict Detection:**
- Existing blocks that don't match blueprint requirements
- Clear error messages with position information
- Graceful termination with proper cleanup

**Placement Failures:**
- Block placement can fail for various reasons
- Inventory management (missing items, wrong items)
- Environmental obstacles or changes
- Bot positioning or movement failures

**Interruption Handling:**
- stopActivity can be set at any time
- Proper cleanup of bot control states
- Graceful termination of all operations
- Potential for resumption from checkpoints

### Performance Considerations

**Loop Optimization:**
- Current implementation uses nested loops for block placement
- State machine should maintain equivalent performance
- Consider chunking or batching operations
- Minimize state transitions during intensive operations

**Memory Management:**
- Blueprint and area objects can be large
- Proper cleanup of resources between phases
- State machine state should be lightweight
- Consider lazy loading of blueprint data

### Integration Patterns

**Existing Interface Compatibility:**
- Maintain doBuild(args) method signature
- Preserve all existing public methods
- Ensure state machine is transparent to callers
- Handle existing error reporting patterns

**Concurrent Operations:**
- Building task runs within larger bot framework
- Must respect stopActivity flags and health checks
- Integration with movement and inventory systems
- Proper resource sharing and synchronization

### Future Extensibility

**Additional Build Features:**
- Support for conditional building logic
- Dynamic blueprint modification during construction
- Parallel building of multiple phases
- Integration with resource gathering systems

**State Machine Enhancements:**
- Hierarchical state machines for complex building scenarios
- Parallel state machines for concurrent operations
- State machine composition for modular building tasks
- Integration with other bot state machines

This comprehensive analysis provides the foundation for converting the building task state machine from its current implicit implementation to an explicit, testable, and maintainable Automat state machine while preserving all existing functionality and adding new capabilities for error recovery and state persistence.-e \n
# Mining Task State Machine Analysis and Conversion Plan

## Overview

This document analyzes the Mining Task State Machine from the Minecraft bot codebase (`mine.py`) and provides a detailed natural language plan for converting it to an explicit state machine using the Automat library. The analysis focuses on the `stripMine()` method which contains the core mining logic.

## 1. Current State Analysis

### How States are Currently Managed Implicitly

The mining system currently manages state through several implicit mechanisms:

**Loop-Based State Management**: The primary state flow is controlled by nested loops:
- An outer `while not self.stopActivity` loop that continues until mining is stopped
- An inner `while area.blocks_mined-area.last_break < area.break_interval` loop that manages work cycles between restocking breaks

**Position-Based State**: The current mining position is tracked through the `z` variable, which represents the depth/distance from the starting point. This acts as both a state indicator and progress tracker.

**Boolean Flag States**: Several flags control behavioral modes:
- `self.speedMode`: Controls whether the bot is in fast exploration mode or normal mining mode
- `self.stopActivity`: Global stop flag that can terminate the entire operation
- Various status flags in the `area` object and `self.dangerType`

**Sequential Code Execution**: The main mining logic follows a rigid sequence of steps within each iteration, with the program counter implicitly determining the current state.

**Status String Tracking**: The system uses string-based status tracking through `area.status` and `self.dangerType` to communicate current conditions.

## 2. State Identification

The mining system can be in the following distinct states:

### RESTOCKING
- **Description**: Bot is at the starting chest, taking inventory and resupplying with tools, torches, and building materials
- **Characteristics**: Inventory operations, checking minimum required items, updating break counters
- **Exit Condition**: Successfully restocked with minimum required items, or failure to obtain minimum items

### WALKING_TO_TUNNEL_FACE
- **Description**: Bot is moving from the chest to the current working position in the tunnel
- **Characteristics**: Pathfinding to specific coordinates, safety checks during movement
- **Exit Condition**: Successfully arrived at target position, or encountered dangerous conditions

### MINING_MAIN_TUNNEL
- **Description**: Bot is clearing the main tunnel blocks at the current depth level
- **Characteristics**: Mining blocks across the tunnel width, tool selection, floor and ceiling mining
- **Exit Condition**: All main tunnel blocks cleared, or encountered unmovable obstacle

### BRIDGING_GAPS
- **Description**: Bot is filling dangerous gaps (air, lava, water) with building blocks
- **Characteristics**: Placing blocks to create safe walking surfaces, safety verification
- **Exit Condition**: All gaps successfully bridged, or unable to bridge critical gaps

### SCANNING_FOR_VALUABLES
- **Description**: Bot is analyzing side areas for valuable ores and minerals
- **Characteristics**: Block scanning in extended ranges, identifying valuable materials
- **Exit Condition**: Scan complete, with or without finding valuables

### MINING_SIDE_TUNNELS
- **Description**: Bot is mining out discovered valuable ores in side passages
- **Characteristics**: Targeted mining of specific valuable blocks, extended tunnel creation
- **Exit Condition**: All accessible valuable blocks mined, or encountered obstacles

### PLACING_TORCH
- **Description**: Bot is placing lighting at appropriate intervals in the tunnel
- **Characteristics**: Torch placement on walls, lighting interval management
- **Exit Condition**: Torch successfully placed, or unable to place torch

### ADVANCING_POSITION
- **Description**: Bot is moving forward to the next depth level in the tunnel
- **Characteristics**: Incrementing position counter, preparing for next cycle
- **Exit Condition**: Position advanced, ready for next work cycle

### UPDATING_PROGRESS
- **Description**: Bot is returning to the starting area to update progress signs and prepare for extended work
- **Characteristics**: Sign updates, status reporting, cycle completion
- **Exit Condition**: Progress updated, ready to begin new cycle or terminate

### ERROR_HANDLING
- **Description**: Bot has encountered a critical error or dangerous condition
- **Characteristics**: Safety shutdown, status reporting, activity termination
- **Exit Condition**: Error resolved or mining operation terminated
...
### External Events (Method Calls)
- **START_MINING**: Initiated by `stripMine()` method call
- **STOP_MINING**: Triggered by setting `self.stopActivity = True`
- **EMERGENCY_STOP**: Triggered by safety checks detecting dangerous conditions

### Internal Events (Condition-Based)
- **RESTOCKING_COMPLETE**: Inventory successfully restocked above minimum thresholds
- **RESTOCKING_FAILED**: Unable to obtain minimum required items
- **POSITION_REACHED**: Successfully arrived at target location
- **MOVEMENT_BLOCKED**: Unable to reach target due to obstacles
- **MAIN_TUNNEL_CLEARED**: All blocks in main tunnel successfully mined
- **MINING_OBSTRUCTED**: Encountered unmovable blocks or dangerous conditions
- **GAPS_BRIDGED**: All dangerous gaps successfully filled
- **BRIDGING_FAILED**: Unable to bridge critical gaps
- **VALUABLES_FOUND**: Ore scanning detected valuable materials
- **NO_VALUABLES_FOUND**: Ore scanning completed without finding valuables
- **SIDE_MINING_COMPLETE**: All accessible valuable blocks extracted
- **TORCH_PLACED**: Lighting successfully installed
- **TORCH_PLACEMENT_FAILED**: Unable to place torch at current location
- **WORK_CYCLE_COMPLETE**: Current depth level fully processed
- **BREAK_INTERVAL_REACHED**: Worked sufficient blocks to trigger restocking break
- **PROGRESS_UPDATED**: Signs and status successfully updated
- **DANGER_DETECTED**: Safety systems detected hazardous conditions
- **TOOL_SHORTAGE**: Critical tools below minimum threshold

### Timer Events
- **BREAK_INTERVAL_TIMER**: Based on `area.blocks_mined - area.last_break` reaching threshold
- **TORCH_INTERVAL_TIMER**: Based on depth progression reaching lighting intervals
...
**From RESTOCKING:**
- → WALKING_TO_TUNNEL_FACE (on RESTOCKING_COMPLETE)
- → ERROR_HANDLING (on RESTOCKING_FAILED)

**From WALKING_TO_TUNNEL_FACE:**
- → MINING_MAIN_TUNNEL (on POSITION_REACHED)
- → ERROR_HANDLING (on MOVEMENT_BLOCKED or DANGER_DETECTED)

**From MINING_MAIN_TUNNEL:**
- → BRIDGING_GAPS (on MAIN_TUNNEL_CLEARED)
- → ERROR_HANDLING (on MINING_OBSTRUCTED or DANGER_DETECTED)

**From BRIDGING_GAPS:**
- → SCANNING_FOR_VALUABLES (on GAPS_BRIDGED)
- → ERROR_HANDLING (on BRIDGING_FAILED)

**From SCANNING_FOR_VALUABLES:**
- → MINING_SIDE_TUNNELS (on VALUABLES_FOUND)
- → PLACING_TORCH (on NO_VALUABLES_FOUND)

**From MINING_SIDE_TUNNELS:**
- → PLACING_TORCH (on SIDE_MINING_COMPLETE)
- → ERROR_HANDLING (on DANGER_DETECTED)

**From PLACING_TORCH:**
- → ADVANCING_POSITION (on TORCH_PLACED or TORCH_PLACEMENT_FAILED)

**From ADVANCING_POSITION:**
- → MINING_MAIN_TUNNEL (on WORK_CYCLE_COMPLETE, if not break time)
- → UPDATING_PROGRESS (on BREAK_INTERVAL_REACHED)

**From UPDATING_PROGRESS:**
- → RESTOCKING (on PROGRESS_UPDATED)
- → ERROR_HANDLING (on critical errors)

**From ERROR_HANDLING:**
- → Terminal state (mining operation ends)

### Conditional Transitions

Several transitions depend on additional conditions:

- **Speed Mode Transitions**: When `self.speedMode` is True, the bot skips certain detailed mining operations
- **Tool Availability**: Some transitions require specific tools to be available
- **Safety Conditions**: All transitions can be interrupted by safety checks
- **Depth Limits**: Operations may terminate when reaching maximum depth

## 5. Output Identification

### State Entry Actions
- **RESTOCKING**: Call `area.restock()`, check minimum items, update counters
- **WALKING_TO_TUNNEL_FACE**: Call `area.walkToBlock3()`, perform safety checks
- **MINING_MAIN_TUNNEL**: Execute `mineColumn()`, `floorMine()`, `ceilingMine()` operations
- **BRIDGING_GAPS**: Execute `bridgeIfNeeded()` for each position
- **SCANNING_FOR_VALUABLES**: Call `findValuables()` for left and right sides
- **MINING_SIDE_TUNNELS**: Execute `mineRow()` with discovered parameters
- **PLACING_TORCH**: Place torch using `safePlaceBlock()`, update torch counter
- **ADVANCING_POSITION**: Increment `z` counter, prepare next iteration
- **UPDATING_PROGRESS**: Update signs with `updateSign()`, walk to start position
- **ERROR_HANDLING**: Set appropriate error status, clean up resources

### Transition Actions
- **Progress Reporting**: Call `self.mineActivity()` with status updates
- **Safety Verification**: Execute `mining_safety_check()` before critical operations  
- **Tool Management**: Wield appropriate tools for upcoming operations
- **Status Updates**: Update `area.status`, `self.dangerType`, and other flags

### Continuous Actions
- **Activity Monitoring**: Check `self.stopActivity` flag throughout operations
- **Progress Tracking**: Update `area.blocks_mined` counter
- **UI Updates**: Refresh activity display with current status

## 6. Conversion Plan

### Phase 1: Define State Machine Structure
1. **Create State Enumeration**: Define all states as clearly named constants
2. **Create Event Enumeration**: Define all events that trigger transitions
3. **Create Automat Machine Class**: Establish the basic state machine framework
4. **Define Initial State**: Set RESTOCKING as the entry point

### Phase 2: Implement Core States
1. **Implement RESTOCKING State**: Extract restocking logic into state entry method
2. **Implement WALKING_TO_TUNNEL_FACE State**: Extract movement logic
3. **Implement MINING_MAIN_TUNNEL State**: Extract main mining logic
4. **Add Basic Transitions**: Connect these three states with their transitions

### Phase 3: Implement Advanced Mining States  
1. **Implement BRIDGING_GAPS State**: Extract gap-filling logic
2. **Implement SCANNING_FOR_VALUABLES State**: Extract ore detection logic
3. **Implement MINING_SIDE_TUNNELS State**: Extract side mining logic
4. **Connect Advanced Transitions**: Wire these states into the flow

### Phase 4: Implement Support States
1. **Implement PLACING_TORCH State**: Extract lighting logic  
2. **Implement ADVANCING_POSITION State**: Extract position management
3. **Implement UPDATING_PROGRESS State**: Extract progress reporting
4. **Implement ERROR_HANDLING State**: Extract error management

### Phase 5: Event System Integration
1. **Replace Flag Checks**: Convert boolean flag checks to event emissions
2. **Add Timer Events**: Implement break interval and torch interval timers
3. **Add Safety Events**: Convert safety checks to event emissions
4. **Add External Events**: Wire up start/stop commands

### Phase 6: Data Context Management
1. **Create State Context Object**: Move shared data (area, z, counters) to context
2. **Implement Context Passing**: Ensure all states can access shared data
3. **Add Context Validation**: Verify data consistency between states
4. **Add Context Persistence**: Support for saving/loading mining progress

### Phase 7: Testing and Validation
1. **Unit Test Individual States**: Test each state's logic in isolation
2. **Integration Test Transitions**: Verify all transitions work correctly
3. **End-to-End Testing**: Test complete mining scenarios
4. **Performance Testing**: Ensure state machine overhead is acceptable

### Phase 8: Migration Strategy
1. **Parallel Implementation**: Run both old and new systems simultaneously
2. **Gradual Cutover**: Switch mining types one at a time to new system
3. **Monitoring and Validation**: Compare outputs between systems
4. **Legacy Cleanup**: Remove old implicit state management code

## 7. Edge Cases

### Safety Handling
- **Lava/Water Detection**: Immediate transition to ERROR_HANDLING state
- **Tool Breakage**: Check tool durability before state transitions
- **Mob Interference**: Handle combat interruptions during mining
- **Server Disconnection**: Preserve state across connection losses

### Resource Management
- **Inventory Full**: Handle full inventory during mining operations
- **Chest Access**: Handle chest being blocked or unavailable
- **Tool Shortage**: Graceful degradation when tools run out
- **Building Material Shortage**: Alternative strategies when bridging materials unavailable

### Pathfinding Issues
- **Blocked Paths**: Alternative routing when primary path is blocked
- **Fall Damage**: Safe movement in vertical mining areas
- **Monster Spawning**: Handle hostile mobs in dark mining areas
- **Structural Collapse**: Handle when mining causes unexpected block updates

### Progress Tracking
- **Inconsistent State**: Recovery when actual world state doesn't match expected
- **Progress Loss**: Handling when mining progress is lost due to external factors
- **Sign Updates**: Handle when signs cannot be updated (blocked, missing, etc.)
- **Coordinate Drift**: Handle when bot position doesn't match expected coordinates

### Mining Specific
- **Silverfish Blocks**: Special handling for infested stone
- **Obsidian Mining**: Tool requirements for extremely hard blocks
- **Gravel Handling**: Managing falling blocks during mining
- **Ore Vein Tracking**: Following irregular ore formations

### System Integration
- **Activity Cancellation**: Clean state machine shutdown when user stops activity
- **Multiple Mining Types**: State machine variations for different mining patterns
- **Speed Mode**: State machine behavior differences in fast exploration mode
- **Error Recovery**: Resuming mining after resolving error conditions
...
- Use `@automat.input` decorators for event methods
- Use `@automat.output` decorators for action methods
- Implement proper state visualization with `automat.core._graphviz`
- Consider using nested state machines for complex sub-operations

### Performance Considerations
- Minimize state transitions during intensive mining operations
- Cache frequently accessed data in state context
- Use efficient event queuing for high-frequency events
- Consider async operations for non-blocking I/O

### Debugging and Monitoring
- Add comprehensive logging for all state transitions
- Implement state machine visualization for debugging
- Add metrics collection for performance monitoring  
- Include state history tracking for error analysis

This conversion plan provides a systematic approach to transforming the implicit state management in the mining system into an explicit, maintainable, and extensible Automat state machine while preserving all existing functionality and handling edge cases appropriately.-e \n
...
WorldState = {
  ...
  health: 0-20,
  food: 0-20,
  health_critical: boolean,        // health <= 50%
  needs_healing: boolean,          // health < 100% or food < 90%
  in_heal_mode: boolean,          
  danger_type: string,            // "health", "food", "mob", etc.
  ...
  activity_phase: string,         // current phase within activity
  ...
  at_farm: boolean,
  at_mine: boolean,
  at_build_site: boolean,
  at_tunnel_face: boolean,        // for mining
  at_start_position: boolean,     // home/base location
  
  // Knowledge/Discovery
  knows_tree_location: boolean,
  knows_ore_location: boolean,
  knows_chest_location: boolean,
  knows_farm_location: boolean,
  tree_validated: boolean,        // 2x2 spruce, height >= 5
  valuables_detected: boolean,    // ores found while mining
  ...
  has_food: boolean,
  has_tools: boolean,
  has_stone_axe: boolean,        // specific tools
  has_pickaxe: boolean,
  has_building_materials: boolean,
  has_torches: number,
  inventory_full: boolean,
  needs_restocking: boolean,
  minimum_items_met: boolean,     // for activities
  
  // Task Progress & State
  trees_chopped: number,
  tree_climbing_height: number,   // current Y position in tree
  tree_at_top: boolean,
  
  crops_harvested: number,
  mature_crops_available: boolean,
  farmland_available: boolean,
  farming_break_counter: number,  // for break intervals
  
  blocks_mined: number,
  tunnel_depth: number,           // Z coordinate in mine
  gaps_need_bridging: boolean,
  torch_interval_reached: boolean,
  mining_obstructed: boolean,
  
  blueprint_phase: number,
  blueprint_loaded: boolean,
  build_area_valid: boolean,
  build_conflicts_detected: boolean,
  materials_analyzed: boolean,
  materials_needed: object,       // {block_type: count}
  ...
  chest_open: boolean,
  chest_accessible: boolean,
  chest_contains_items: boolean,
  chest_has_space: boolean,
  chest_type: string,            // "storage", "minecart", etc.
  ...
  command_issuer: string,
  awaiting_response: boolean,
  following_player: boolean,
  player_nearby: boolean,
  
  // Work Area State
  work_area_defined: boolean,
  work_area_valid: boolean,
  work_progress: number,         // percentage 0-100
  break_interval_reached: boolean,
  
  // Error & Recovery State
  error_state: boolean,
  error_type: string,
  recovery_attempted: boolean,
  operation_retries: number


  // Main Activity State Machine
  activity_major: boolean,              // true if major activity running
  activity_last_duration: number,       // duration of last completed activity
  activity_state: string,               // IDLE, STARTING, ACTIVE, STOPPING
  
  // Wood Chopping State Machine
  tree_height: number,                  // height of current tree
  tree_base_x: number,                  // base coordinates of tree
  tree_base_z: number,
  tree_bounding_box: object,            // {x_min, x_max, y_min, y_max, z_min, z_max}
  chopping_state: string,               // SEARCHING, VALIDATING, CLIMBING, DESCENDING
  chopping_spiral_position: number,     // current position in spiral pattern
  tree_search_radius: number,           // search area for trees
  
  // Farming State Machine  
  farming_state: string,                // HARVESTING, PLANTING, DEPOSITING, RESTING
  long_break_needed: boolean,           // needs extended rest period
  planting_locations_available: boolean,
  harvest_cycle_count: number,          // iterations in current harvest cycle
  planting_cycle_count: number,         // iterations in current planting cycle
  break_interval: number,               // blocks before break (farming)
  
  // Chest Interaction State Machine
  chest_state: string,                  // UNINITIALIZED, CLOSED, OPEN, ERROR
  chest_object_found: boolean,          // physical chest located
  chest_container_valid: boolean,       // container connection established
  chest_operation_pending: string,      // current operation type
  chest_retry_count: number,            // connection retry attempts
  
  // Survival State Machine
  survival_state: string,               // MONITORING, RECOVERING
  health_percentage: number,            // 0-100
  food_percentage: number,              // 0-100
  last_health_check: number,            // timestamp
  recovery_start_time: number,          // when healing began
  
  // Building Task State Machine
  building_state: string,               // BLUEPRINT_LOADING, AREA_VALIDATION, etc.
  blueprint_name: string,               // current blueprint
  blueprint_exists: boolean,            // blueprint file found
  build_stage: number,                  // current building phase (1, 2, 3...)
  block_placement_x: number,            // current placement position
  block_placement_y: number,
  block_placement_z: number,
  block_placement_progress: number,     // blocks placed in current phase
  sneak_required: boolean,              // for interactive blocks
  jump_required: boolean,               // for high placements
  build_analysis_complete: boolean,     
  
  // Mining State Machine
  mining_state: string,                 // RESTOCKING, MINING_MAIN_TUNNEL, etc.
  speed_mode: boolean,                  // fast exploration mode
  mining_break_interval: number,        // blocks before restocking
  last_break_position: number,          // Z coordinate of last break
  side_tunnel_active: boolean,          // currently mining side passage
  side_tunnel_direction: string,        // "left" or "right"
  mining_pattern: string,               // "strip", "branch", "quarry"
  torch_placement_counter: number,      // blocks since last torch
  torch_interval: number,               // blocks between torches
  mining_area_status: string,           // current area condition
  main_tunnel_cleared: boolean,         // for current depth
  
  // Bot Control States
  is_sneaking: boolean,                 // shift key held
  is_jumping: boolean,                  // space key held
  is_sprinting: boolean,                // sprint mode active
  is_flying: boolean,                   // creative flight mode
  
  // Tool Durability & Equipment
  tool_durability: object,              // {pickaxe: 0-100, axe: 0-100, etc.}
  armor_durability: object,             // {helmet: 0-100, chest: 0-100, etc.}
  active_tool: string,                  // currently wielded tool
  tool_switch_needed: boolean,          
  
  // Network & Connection
  connection_stable: boolean,           
  last_server_response: number,         // timestamp
  pending_operations: number,           // queued network operations
  
  // Performance & Metrics
  fps: number,                          // frames per second
  tps: number,                          // ticks per second
  operation_duration: object,           // {operation_name: duration_ms}
  state_transition_count: number,       // total transitions
  
  // State Machine States (for GOAP planning)
  active_state_machines: array,         // list of running FSMs
  state_history: array,                 // recent state transitions
  pending_transitions: array,           // queued state changes
  
  // Environmental Details
  light_level: number,                  // 0-15
  biome: string,                        // current biome type
  dimension: string,                    // overworld, nether, end
  nearby_blocks: object,                // counts of block types nearby
  
  // Task Queuing & Scheduling
  task_queue: array,                    // pending tasks
  task_priority: object,                // {task_id: priority}
  task_dependencies: object,            // {task_id: [dependency_ids]}
  
  // Advanced Mining/Building
  blueprint_conflict_positions: array,  // [{x,y,z,expected,actual}]
  ore_vein_positions: array,           // discovered ore locations
  bridge_positions_needed: array,      // gaps requiring bridging
  
  // Resource Tracking
  resource_consumption_rate: object,   // {resource: rate_per_minute}
  estimated_task_duration: number,     // for current task
  resource_sufficiency: object,        // {resource: boolean}

  // Additional Main Activity States
  activity_stopping: boolean,           // in process of stopping activity
  activity_starting: boolean,           // in process of starting activity
  activity_completed: boolean,          // activity finished successfully
  activity_failed: boolean,             // activity failed with error
  can_start_major_activity: boolean,    // no major activity currently running
  
  // Additional Wood Chopping States
  chopping_preparation_complete: boolean,  // ready to start climbing
  chopping_cycle_position: number,         // position in 4-block climb cycle
  chopping_access_cleared: boolean,        // initial blocks removed
  tree_search_failed: boolean,             // no suitable trees found
  at_tree_top: boolean,                    // reached climbing termination
  descent_level: number,                   // current Y during descent
  
  // Additional Building States
  building_restocking: boolean,            // gathering materials phase
  construction_in_progress: boolean,       // actively placing blocks
  phase_completion_pending: boolean,       // current phase done, preparing next
  build_terminating: boolean,              // ending build operation
  block_conflict_found: boolean,           // wrong block in blueprint position
  blocks_placed_this_phase: number,        // progress in current phase
  total_blocks_this_phase: number,         // total blocks needed in phase
  
  // Additional Mining States  
  walking_to_mine: boolean,                // traveling to tunnel face
  scanning_for_ores: boolean,              // checking for valuables
  mining_side_tunnel: boolean,             // extracting found ores
  placing_torch_needed: boolean,           // torch placement due
  advancing_tunnel: boolean,               // moving to next position
  updating_mine_progress: boolean,         // updating signs/status
  mine_error_state: boolean,               // critical mining error
  ores_found_left: array,                  // valuable blocks on left side
  ores_found_right: array,                 // valuable blocks on right side
  side_tunnel_depth: number,               // depth into side passage
  main_tunnel_width: number,               // tunnel dimensions
  main_tunnel_height: number,
  
  // State Machine Metadata
  previous_state: object,                  // {machine: string, state: string}
  state_entry_time: object,                // {machine_state: timestamp}
  failed_transitions: array,               // [{from, to, reason, timestamp}]
  state_machine_errors: object,            // {machine: error_details}
  
  // Goal Planning States (for GOAP)
  active_goal: string,                     // current highest priority goal
  goal_plan: array,                        // sequence of actions to achieve goal
  plan_progress: number,                   // current step in plan
  goal_interrupted: boolean,               // plan interrupted by higher priority
  replan_needed: boolean,                  // world state invalidated plan
  
  // Action Preconditions & Effects Tracking
  action_preconditions_met: object,        // {action: boolean}
  expected_effects: object,                // predicted world state changes
  actual_effects: object,                  // observed world state changes
  
  // Complex State Combinations
  can_harvest: boolean,                    // has tools && mature_crops_available
  can_plant: boolean,                      // has_seeds && farmland_available
  can_mine: boolean,                       // has_pickaxe && !mining_obstructed
  can_build: boolean,                      // materials_needed satisfied
  can_chop: boolean,                       // has_axe && knows_tree_location
  
  // Interruption & Recovery
  interruption_reason: string,             // why activity was interrupted
  recovery_action_available: boolean,      // can recover from current error
  checkpoint_saved: boolean,               // state saved for resumption
  checkpoint_data: object,                 // saved state information
  
  // Resource Generation Tracking
  wood_generation_rate: number,            // logs per minute
  ore_generation_rate: number,             // ores per minute
  crop_yield_rate: number,                 // crops per harvest cycle
  
  // Safety & Risk Assessment
  risk_level: number,                      // 0-10 danger assessment
  escape_route_available: boolean,         // path to safety exists
  defensive_position_available: boolean,   // safe spot nearby
  
  // Coordination States (for multi-FSM)
  fsm_conflicts: array,                    // conflicting state machine requests
  fsm_dependencies_met: object,            // {fsm: boolean}
  shared_resource_locks: object,           // {resource: owning_fsm}
  
  // Time-based States
  activity_timeout_approaching: boolean,   // nearing time limit
  idle_duration: number,                   // time since last activity
  last_player_interaction: number,         // timestamp of last command
  scheduled_task_due: boolean,             // scheduled activity ready
  
  // Special Mining States
  bedrock_reached: boolean,                // hit world bottom
  cave_system_found: boolean,              // intersected natural cave
  mineshaft_found: boolean,                // found generated structure
  ravine_detected: boolean,                // dangerous drop nearby
  
  // Special Building States
  building_on_slope: boolean,              // terrain requires adjustment
  water_in_build_area: boolean,            // requires drainage
  existing_structure_conflict: boolean,    // other builds in the way
  
  // Memory & Learning (for GOAP optimization)
  successful_plans: array,                 // plans that achieved goals
  failed_plans: array,                     // plans that didn't work
  action_success_rate: object,             // {action: success_percentage}
  common_plan_patterns: array,             // frequently used sequences
  
  // Debug & Development States
  verbose_logging: boolean,                // detailed state logging
  state_visualization_enabled: boolean,    // graph generation active
  performance_profiling: boolean,          // timing all operations
  simulation_mode: boolean                 // dry-run mode for testing


  // Main Activity State Machine - Additional States
  activity_name: string,                   // specific activity name (not just type)
  activity_queue: array,                   // queued activities waiting to start
  minor_command_active: boolean,           // non-major activity in progress
  command_argument_valid: boolean,         // command has required arguments
  follow_target: string,                   // player name being followed
  
  // Wood Chopping State Machine - Missing States
  chopping_state_detailed: string,         // IDLE, SEARCHING_FOR_TREE, VALIDATING_TREE, 
                                          // PREPARING_TO_CLIMB, CLIMBING_AND_CUTTING,
                                          // DESCENDING_AND_CUTTING, COMPLETED, ERROR
  spiral_face: string,                     // "south", "east", "north", "west"
  tree_invalid_reason: string,             // why tree validation failed
  chopping_completed: boolean,             // successfully chopped entire tree
  chopping_error: string,                  // specific error during chopping
  
  // Farming State Machine - Additional States
  farming_state_detailed: string,          // includes TERMINATED state
  farm_work_found: boolean,                // work available in current state
  restocking_from_chest: boolean,          // currently at chest restocking
  farming_equipment_list: array,           // required items for farming
  
  // Building Task State Machine - Missing States
  building_state_detailed: string,         // BLUEPRINT_LOADING, AREA_VALIDATION, ANALYSIS,
                                          // RESTOCKING, CONSTRUCTION, PHASE_COMPLETION, 
                                          // TERMINATION
  blueprint_loading: boolean,              // currently loading blueprint file
  area_validating: boolean,                // validating work area
  analyzing_blocks: boolean,               // comparing existing vs required blocks
  phase_transitioning: boolean,            // moving between build phases
  build_complete: boolean,                 // all phases completed
  no_more_phases: boolean,                 // no blueprint for next stage
  placement_strategy: string,              // "top", "front", "special"
  bot_placement_position: object,          // {x, y, z} for bot during placement
  
  // Mining State Machine - Missing States
  mining_state_detailed: string,           // RESTOCKING, WALKING_TO_TUNNEL_FACE,
                                          // MINING_MAIN_TUNNEL, BRIDGING_GAPS,
                                          // SCANNING_FOR_VALUABLES, MINING_SIDE_TUNNELS,
                                          // PLACING_TORCH, ADVANCING_POSITION,
                                          // UPDATING_PROGRESS, ERROR_HANDLING
  bridging_in_progress: boolean,           // currently filling gaps
  bridge_positions_completed: array,       // successfully bridged locations
  column_mining_active: boolean,           // mining vertical column
  floor_mining_active: boolean,            // mining floor blocks
  ceiling_mining_active: boolean,          // mining ceiling blocks
  mining_work_area: object,                // current mining area boundaries
  sign_update_needed: boolean,             // progress signs need updating
  mining_terminated: boolean,              // mining ended (success or failure)
  
  // Chest Interaction - Additional States
  chest_connection_pending: boolean,       // waiting for container connection
  chest_operation_in_progress: boolean,    // deposit/withdraw active
  container_reference: object,             // current container object
  chest_error_recovery_available: boolean, // can attempt recovery
  
  // Inter-State Machine Communication
  state_machine_priorities: object,        // {machine_name: priority_level}
  pending_state_machine_switches: array,   // transitions between major activities
  activity_handoff_data: object,           // data passed between activities
  
  // GOAP-Specific World State Properties
  goals_available: array,                  // all possible goals given current state
  goal_costs: object,                      // {goal: estimated_cost}
  actions_available: array,                // actions whose preconditions are met
  action_costs: object,                    // {action: cost}
  world_state_hash: string,                // for efficient plan caching
  
  // Missing Environmental States
  blocks_in_reach: array,                  // blocks within interaction range
  entities_nearby: object,                 // {entity_type: count}
  chunk_loaded: boolean,                   // current chunk fully loaded
  
  // Missing Resource States
  equipment_slots: object,                 // {slot: item} for hotbar/armor
  consumables_available: object,           // {type: count} for food/potions
  crafting_possible: array,                // items that can be crafted now
  
  // Missing Timing States
  state_duration: object,                  // {state: milliseconds} time in each state
  transition_timestamps: object,           // {from_to: timestamp} for transitions
  activity_efficiency: object,             // {activity: blocks_per_minute}
  
  // Missing Validation States
  preconditions_checked: boolean,          // validated before state entry
  postconditions_met: boolean,             // state exit conditions satisfied
  invariants_valid: boolean,               // state machine invariants holding
  
  // Activity-Specific Completion Flags
  mining_goals_achieved: boolean,          // mined target amount/depth
  building_goals_achieved: boolean,        // built all phases
  farming_quota_met: boolean,              // harvested enough crops
  wood_quota_met: boolean,                 // collected enough wood
  
  // Complex Derived States (for GOAP)
  ready_for_major_activity: boolean,       // health good, not busy, equipped
  exploration_viable: boolean,             // can safely explore new areas
  construction_viable: boolean,            // has materials and location
  resource_gathering_needed: boolean,      // low on critical resources
  
  // State Machine Synchronization
  waiting_for_state_machine: string,       // blocked on another FSM
  state_machines_synchronized: boolean,    // all FSMs in compatible states
  cross_fsm_transaction: object,          // atomic multi-FSM operation
  
  // Error Classification
  recoverable_error: boolean,              // can be resolved automatically
  error_recovery_attempts: number,         // times tried to recover
  error_requires_user: boolean,            // needs human intervention
  last_known_good_state: object           // for rollback capability
}
